...
`TP/FP/TN/FN 1,250/8/1,288/46`에서 `1,278/8/1,288/18`이 됐다. Raw FN 18건은 그대로
보존했고 fixture annotation도 바꾸지 않았다. 별도 처분 장부가 18건을 모두 검증하므로
미분류 FN은 0건이다.
...
Strict와 contract-adjusted confusion matrix는 reclassified case가 0건이라 같다.
...
| 처분 | 건수 | case |
| --- | ---: | --- |
| structurally-unresolvable | 1 | `이다→걸까`: 축약 뒤 지정사에 대응하는 양의 byte span이 없음 |
| gold-or-adapter | 7 | `이→이중`, `무어→무언가`, `잠식당→잠식당하기`, `가다→올라가`, `나다→생겨나`, `오다→들어와서는`, `누구→누군가가` |
| out-of-contract | 9 | 별도 부사 파생 5건, `밀다→밀려`, 무공백 합성·경계 3건 |
| cost-prohibitive | 1 | `하→책임하에서`: 다품사 한 음절 내부 검색에 범용 문맥 분석이 필요 |
| dictionary-required | 0 | 두 기본 사전이 합의한 안전한 alias가 남아 있지 않음 |
...
없으므로 이 case들을 `dictionary-required`로 두지 않았다.
