...
동일한 7개 명시적 품사 질의를 full-POS any/smart 형태 검색, 활용형 열거 정규식과 짧은 어간
정규식으로 실행한다.
...
Kfind는 any와 smart를 품질·실행시간에서 각각 독립된 행으로 보존한다.
...
source component를 contract positive로, gold 정렬 오류를 contract negative로 교정한다.
비문·비표준 입력만 분모에서 제외하며 비용, profile과 미구현 표준 문법은 제외하지 않는다.
kfind와 외부 제품의 모든 품질 표·차트는 raw와 contract-adjusted를 나란히 표시하고 두 지표를
합치거나 raw 오류를 숨기지 않는다. Contract review가 없는 fixture는 raw confusion matrix를
contract-adjusted 값으로 복사하고 `reviewed_cases=0`을 기록한다. 성능 지표에는 정답 계약을
적용하지 않으며 initialization, 처리량, 지연 시간과 RSS를 workload별 단위로 기록한다.
Site의 Canonical, query matrix와 Robust explicit-POS 절은 각각 kfind
`embedded/full-POS × any/smart` 4개 profile과 외부 분석기 고정 설정을 같은 순서로 표시한다.
각 절은 F1 chart, raw·contract-adjusted confusion matrix와 같은 fixture의 성능 표를 함께
제공한다. Workload가 다른 행을 합치거나 성능 단위를 공통 점수로 환산하지 않는다.
...
Case ID·문장·query·품사·gold span을 유지한 채 contract review annotation만 적용한 fixture는
적용 전 fixture SHA-256을 외부 스냅샷 정합성에 사용하고, 적용 후 SHA-256을 품질 report
identity로 사용한다.
...
`site-morphology.json`은 공개 site의 세 morphology workload가 소비하는 profile별 품질·성능과
원본 report의 revision, SHA-256만 보존한다. Site는 이 JSON의 raw와 contract-adjusted 값을
D3로 렌더링하고 같은 profile 순서로 성능 표를 만든다. 승인
보고서와 site snapshot을 갱신할 때 같은 변경에서 다시 생성한다.
...
코드 작업 평가는 고정 revision의 수정 전 입력에서 검색, 파일 열람, 수정, 원본 테스트의
실패·성공까지 기록한다. 같은 과제의 검색 조건마다 독립된 입력 복사본을 사용한다.
일반 검색 조건에도 어간·동의 표현·식별자 질의와 후속 검색을 허용하며, kfind 조건은
형태 검색에 kfind를 사용하고 정확한 문자열·식별자 검색에 일반 도구를 함께 쓸 수 있다.

과제별 요구사항과 검증 명령을 수행 전에 고정하고, 원본 테스트·검증기를 수정해 성공을
만들지 않는다. 검색 결과 유무와 작업 완료 여부를 구분한다. 검색·열람 횟수, 실제 반환
byte 수, 도구 실행 시간, 최종 patch와 검증 출력을 원본 기록에 보존한다.
도구 실행 시간에는 모델의 판단 시간이나 호출 간 대기 시간이 포함되지 않는다.

동일 에이전트의 순차 수행은 앞선 조건의 답을 알게 되는 비맹검 pilot이다. 실행 순서를
교대하고 사전 파일 열람 여부를 공개한다. 기록된 작업 성공은 보고할 수 있지만 독립적인
모델 성공률 비교로 해석하지 않는다. 검색 명령이나 patch의 재실행은 재현 검증이며 새로운
에이전트 trial로 세지 않는다. 측정하지 않은 모델 token·비용은 수치로 추정하지 않는다.
