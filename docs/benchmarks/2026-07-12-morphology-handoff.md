...
- kfind 비용: 16,975.4 cases/s, p95 0.1333 ms, peak RSS 5.0 MiB (5회 median)
...
다음 용언 개선 단위는 full-POS dev의 `바쁘니`, `얻으리라고`를 근거로 일반
`-(으)니`와 제한된 `-(으)리라고` 연쇄를 추가한다. core lexicon에 표제어를 추가하지
않고, 다른 후속 어미를 소비하지 않으며, test baseline과 hard-negative precision을 유지한다.
...
1. full-POS의 남은 용언 `continuation-rejected`인 `바쁘니`, `얻으리라고`를 우선 검증한다.
...
5. 다음 제품 판정용 unseen source를 먼저 고정한다.
6. 그 전에는 P3 filtering과 기본 검색 결과를 변경하지 않는다.
