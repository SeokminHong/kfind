...
- Rust/WASM engine은 component를 자동 초기화하지 않는다. 생성자에서 선택적으로 받거나 생성 후
  수동 load할 수 있으며, resource 없는 component smart compile은 명시적 오류다. 손상된 수동
  교체는 기존에 검증된 resource를 보존한다.
...
compact schema 1 owning loader, 제품 `NominalComponent` accept 복구, optional/manual
