# 명사 smart-boundary 계약

## 제품 동작

`smart`는 문자열 token 경계뿐 아니라 검증된 형태 분석의 완전한 명사 component span도 검색
결과로 인정한다. query가 component의 prefix, suffix 또는 내부에 있어도 양쪽 경계를 형태
분석으로 증명해야 한다.
...
- `NominalComponent` branch가 있는 `smart` plan만 compact component resource를 사용한다.
