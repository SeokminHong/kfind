...
공식 constraint evaluator는 compact decision과 정책 결과만 결과 객체에 보관한다. decision·diagnostic parity를 검증한 상세 proof는 후보 평가 직후 해제하고 benchmark JSON에 복제하지 않으며, node·path proof가 필요한 명시적 진단은 graph shadow 경로가 담당한다. 따라서 evaluator peak RSS와 처리량은 diagnostic serialization이나 corpus 전체 proof 보관이 아니라 제품 후보의 bounded 수명을 측정한다.
