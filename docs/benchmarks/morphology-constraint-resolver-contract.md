...
제품 hot path는 proof 전체를 생성하는 `ConstraintResolution`과 분리한 compact `ConstraintDecision`을 사용한다. decision은 outcome, pattern별 안정된 support kind와 ambiguity class만 보존하며 node 문자열, complete witness path와 진단용 continuation unit을 복제하지 않는다. 명시적 진단 요청만 같은 prepared graph에서 proof를 materialize하고 decision과 outcome·수용 결과가 일치해야 한다.
