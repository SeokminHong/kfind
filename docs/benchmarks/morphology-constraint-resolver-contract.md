...
`ConstraintResolver`는 먼저 `SupportedAnalysisSet`을 만든다. 각 `SupportedAnalysis`는 pattern index, lexical evidence, span relation, continuation 전이, context proof와 complete path를 포함한다. 같은 증거 mask의 경로도 source analysis identity나 continuation·context proof가 다르면 버리지 않는다.
...
3. policy quality: `whole`, `explicit-component`, `possible-analysis` 각각의 precision, recall, hard-negative 결과를 측정한다.
