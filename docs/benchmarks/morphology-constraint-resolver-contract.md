...
`PreparedTokenAnalysis`는 정규화된 현재 token의 source node, categorical edge와 reachability를 한 번 만들고 같은 token의 모든 candidate와 pattern 평가가 공유한다. nominal-particle host처럼 현재 token 구조만으로 정해지는 context fact는 이를 소비할 수 있는 candidate가 처음 요청할 때 prepared token summary에 가능한 range 집합으로 한 번 계산하며 후보 없음과 복수 후보를 같은 값으로 접지 않는다. candidate decision은 자신의 core·consumed 범위와 일치하는 fact만 소비하고 complete witness path를 다시 열거하지 않는다. 이전·다음 token graph는 실제 adjacent constraint가 요구할 때만 준비한다. context selector는 token 동일성·인접 constraint와 complete-path POS 존재 여부를 먼저 검사하고 충족 가능한 selector만 상세 graph traversal로 검증하며, 충족 불가능한 반복 token과 copular frame을 찾기 위해 complete path를 열거하지 않는다. cache는 resource identity와 정규화 token을 key로 사용하고 고정 entry·byte 상한을 가지며 corpus 크기에 비례해 증가하지 않는다.

known node의 surface와 POS는 schema 4 string table과 정규화 token을 빌려 쓰며 candidate마다 소유 문자열로 복제하지 않는다. categorical transition은 resource가 부여한 정수 POS class와 dense `end_pos -> start_pos` bit matrix로 조회한다. compact decision은 query-constrained traversal에서 outcome과 policy projection에 필요한 support 식별자를 직접 수집하고 중간 candidate path 집합을 다시 pattern별로 분석하지 않는다. continuation traversal은 시작 lexical path의 unit stream을 한 번 만들고 successor unit을 제자리에서 추가·되돌리며 부분 path마다 unit stream과 support를 다시 만들지 않는다. prefix 판정은 borrowed suffix view의 coverage와 POS를 직접 소비하고 최종 지지 proof가 채택될 때만 continuation unit을 materialize한다. compact decision에 필요 없는 reverse edge, `SupportedAnalysis`, continuation unit과 witness path는 같은 prepared graph에서 진단을 요청할 때만 materialize한다.
