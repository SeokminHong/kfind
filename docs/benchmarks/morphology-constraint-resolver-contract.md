...
공식 constraint evaluator의 성능 process는 compact decision과 정책 결과만 만들고 상세 proof를 생성하거나 결과 객체에 보관하지 않는다. 같은 fixture를 읽는 별도 diagnostic parity process가 `ConstraintResolution`을 materialize해 decision과 outcome·정책 결과를 비교하며 이 process의 시간과 peak RSS는 제품 후보 성능에서 제외한다. node·path proof가 필요한 명시적 진단은 graph shadow 경로가 담당한다.
