...
known complete path가 없으면 resolver는 candidate별로 query core 시작 전의 prefix만 unknown node로 연결한 hybrid path를 만들 수 있다. core를 덮는 lexical identity와 core 이후의 anchor·continuation·context proof는 모두 source node여야 하며 unknown node는 lexical surface, POS 또는 DFA 전이의 근거가 되지 않는다. 이 제한을 만족하는 token 전체 witness가 없으면 `UnknownOnly` 또는 `NoCompletePath`를 반환한다.
