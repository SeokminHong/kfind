...
known node의 surface와 POS는 schema 4 string table과 정규화 token을 빌려 쓰며 candidate마다 소유 문자열로 복제하지 않는다. categorical transition은 `end_pos -> start_pos` 색인으로 조회한다. compact decision은 query-constrained traversal에서 outcome과 policy projection에 필요한 support 식별자를 직접 수집하고 중간 candidate path 집합을 다시 pattern별로 분석하지 않으며, compact decision에 필요 없는 reverse edge, `SupportedAnalysis`, continuation unit과 witness path는 같은 prepared graph에서 진단을 요청할 때만 materialize한다.
