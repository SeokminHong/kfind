...
`PreparedTokenAnalysis`는 정규화된 현재 token의 source node, categorical edge와 reachability를 한 번 만들고 같은 token의 모든 candidate와 pattern 평가가 공유한다. 이전·다음 token graph는 실제 adjacent constraint가 요구할 때만 준비한다. cache는 resource identity와 정규화 token을 key로 사용하고 고정 entry·byte 상한을 가지며 corpus 크기에 비례해 증가하지 않는다.
