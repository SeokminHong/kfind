...
사람이 `--pos`와 atom 태그를 생략하는 사용법은 별도 1,000-case fixture로 측정했다. positive는
같은 held-out gold span을 사용하고, negative는 query 표제어가 지원하는 어떤 품사로도 없는
문장이다. 명시적 품사 task와 정답 정의가 다르므로 F1 순위를 합치지 않는다.

![무품사 검색 품질](assets/smart-component-human-untagged-quality.svg)

| lexicon | boundary | TP / FP / FN | precision | recall | F1 | init | cases/s | p95 | RSS |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| embedded | smart | 315 / 1 / 185 | 99.68% | 63.0% | 77.21% | 279.46 ms | 15,383.8 | 0.2672 ms | 51.0 MiB |
| embedded | any | 360 / 10 / 140 | 97.30% | 72.0% | 82.76% | 1.10 ms | 28,529.2 | 0.1358 ms | 5.3 MiB |
| full-POS | smart | 410 / 1 / 90 | 99.76% | 82.0% | 90.01% | 421.63 ms | 7,899.4 | 0.3698 ms | 92.1 MiB |
| full-POS | any | 474 / 13 / 26 | 97.33% | 94.8% | 96.05% | 141.52 ms | 12,919.8 | 0.2057 ms | 46.5 MiB |

| lexicon | 기대 품사 plan 포함 | multi-POS plan | literal fallback |
| --- | ---: | ---: | ---: |
| embedded | 46.8% (234/500) | 57.0% (285/500) | 28.2% (141/500) |
| full-POS | 96.4% (482/500) | 44.2% (221/500) | 1.0% (5/500) |

품사를 생략하면 full-POS가 query 분석 coverage를 담당한다. 따라서 사람용 기본 경로는
full-POS와 `smart`를 유지한다. `any`는 recall 우선 재검색 옵션이며 FP 증가를 감수한다.
반대로 에이전트는 품사를 직접 지정해 forced analysis를 만들 수 있으므로 full-POS 없이
embedded와 `any`를 사용할 수 있다. 이 차이가 명시적 품사 benchmark에서 embedded/full-POS
`any`의 품질이 같고, 무품사 benchmark에서는 다른 원인이다.
...
지정사 dev gold-aligned lattice candidate 1,007건 중 reject 50건의 구조별 분류는
[지정사 lattice dev gold 진단](2026-07-13-copula-dev-diagnosis.md)을 기준으로 한다. 제품 판정은
UD Korean-PUD r2.18의 밀봉된 fixture로 검증하며 Kaist·KSL test, Korean-GSD와 PUD 결과에 맞춰
비용이나 threshold를 바꾸지 않는다.
