...
- 측정일: 2026-07-24
- revision: `f5fb19d0bd8213b6fc7e6575cc5ad3054dea8cdf`
...
Kfind는 raw positive 56건을 모두 회수했고 strict negative에서 FP 6건을 허용했다. 이 6건은
같은 품사 동형 활용 4건과 source에 정렬된 내부 성분·구조적으로 분리하지 않는 동형 활용
2건이다. 고정 contract review를 같은 예측에 적용하면 TPᶜ/TNᶜ/FPᶜ/FNᶜ는
62/50/0/0이다. Contract-adjusted는 별도 실행 모드나 후처리가 아니다.

활용형 열거 정규식은 raw FN 6건과 FP 8건, 짧은 어간 정규식은 FN 10건과 FP 34건이었다.
어간을 짧게 열면 recall은 유지되지만 경계 밖 문자열을 함께 잡는 비용이 커졌다. 이 fixture에서
kfind raw F1은 열거 정규식보다 7.20%p, 짧은 어간 정규식보다 27.27%p 높다.

실행시간은 `rg`가 두 정규식에서 각각 64.10 ms와 66.43 ms로 가장 짧았다. Kfind는
2311.37 ms, BSD `grep`은 열거 정규식 5264.31 ms와 어간 정규식 1789.18 ms였다. 이는
13.54 MiB 단일 파일을 7개 fresh process가 각각 스캔한 batch 시간이다. 정규식 도구와
pattern에 따라 결과가 크게 달라지므로 품질과 시간을 하나의 점수로 합치지 않는다.
...
- kfind: 1.0.0-rc.1, binary
  `d2c1ab1a54d8cc3e2f2be17b7e4a2bccf477e195c164b8ad8e935ab82740df7a`
- full POS:
...
| ---------------- | --------------: | ------------------: | --------------: | --------------------: |
| kfind            |       56/50/6/0 | 90.32/100.00/94.92% |       62/50/0/0 | 100.00/100.00/100.00% |
...
| --------------- | ---------: | ---------: | ---------: | ---------: | --------------: |
| kfind           | 2311.37 ms | 2300.24 ms | 2464.53 ms | 2464.53 ms |            41.0 |
| rg enumerated   |   64.10 ms |   63.65 ms |   67.94 ms |   67.94 ms |          1478.0 |
| grep enumerated | 5264.31 ms | 5236.44 ms | 5372.81 ms | 5372.81 ms |            18.0 |
| rg stem         |   66.43 ms |   63.87 ms |   74.43 ms |   74.43 ms |          1426.2 |
| grep stem       | 1789.18 ms | 1779.27 ms | 1902.56 ms | 1902.56 ms |            53.0 |

## Query별 raw F1

| query        |   kfind | enumerated regex | stem regex |
| ------------ | ------: | ---------------: | ---------: |
| `v:걷다`     |  84.21% |           75.00% |     66.67% |
| `v:듣다`     |  84.21% |           77.78% |     69.57% |
| `v:돕다`     | 100.00% |           85.71% |     80.00% |
| `v:짓다`     | 100.00% |           93.33% |     66.67% |
| `v:부르다`   | 100.00% |           94.12% |     55.56% |
| `adj:예쁘다` | 100.00% |          100.00% |     80.00% |
| `v:되다`     | 100.00% |           88.89% |     58.82% |
...
`9fd133df01b5ee2dfbe29300d40ba193c94d1737c36ba613600ed4d514da1524`다. JSON에는 정규식,
