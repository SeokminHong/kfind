...
development 명시적 품사 `smart`는 embedded TP 442 / FP 2 / FN 58, full-POS TP 441 / FP 2 /
FN 59다. 사전 분석이 없는 coarse `noun` fallback은 embedded에서 더 많은 의존명사·고유명사
경로를 보존한다. `token`과 `any`에서는 두 lexicon profile의 품질이 같다. 세부 품사와 품질 계약은
[User smart precision 품질·성능](2026-07-14-user-smart-precision.md), 현재 처리량과 latency는
[의존명사 coarse-POS fallback recall](2026-07-14-dependent-noun-recall.md)을 기준으로 한다.
...
- `ending.connective-ji` left-edge의 bounded token 판정은 `없다/VA -> 없지는`과 같은 candidate
  ...
  오른쪽 token 경계를 유지한다.
- 남은 full-POS `smart` FN 59건은 `boundary-rejected` 42건, `surface-missing` 11건,
...
1. development full-POS `smart`의 TP 441 / FP 2 / FN 59를 기준선으로 사용한다. embedded는
   `197명이`를 복구했지만 full-POS는 기존 `명/NNG` 분석 때문에 의존명사 fallback을 만들지 않는다.
2. `서사극이라`와 `인쇄업자가`는 같은 candidate surface 대조군에서 exact component 경로가
   positive와 negative를 구분하지 못했으므로 제품에 열지 않는다. full-POS의 coarse `noun`
   세부 품사 모호성을 넓히려면 기존 사전 분석과 대체 분석의 union을 먼저 shadow로 측정하고
   `명 -> 익명이`를 포함한 같은 표면형 hard negative를 유지한다.
3. development에서 FN 59 미만, precision 99.00% 이상과 기존 hard-negative 신규 FP 0을 모두
