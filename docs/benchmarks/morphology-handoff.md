...
- `copula-lattice` 후보는 밀봉된 PUD 제품 gate에서 탈락했다. 공개 제품 옵션을 추가하지 않고
  지정사 homonym union을 유지한다. lattice 판정은 benchmark shadow 계측에만 남긴다.
...
- Korean-Kaist·KSL dev의 gold-aligned lattice candidate 1,007건 중 50건이 reject다. 주원인은
  ...
  같다.
- UD Korean-PUD r2.18의 921-case sealed fixture에서 `copula-lattice` 투영은 precision 100.00%,
  recall 65.37%, TP 285, FP 0, FN 151이었다. 80.00% recall gate를 통과하지 못했다.
- 같은 fixture의 union은 precision 90.06%, recall 72.71%, TP 317, FP 35, FN 119였다. lattice는
  false positive 35개와 함께 true positive 32개를 제거했다. embedded와 full-POS 결과는 같다.
...
PUD unseen backend 평가는 완료되었으며 반복 실행하지 않는다. 실행 계약과 결과 digest는
[unseen 제품 판정](2026-07-13-copula-unseen-evaluation.md)에 고정한다.
