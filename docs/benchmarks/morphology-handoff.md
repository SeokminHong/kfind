...
다음 recall 작업은 남은 수사 FN인 `수십만`, `십일월`, `백명`의 완성된 `NR` 연쇄와 뒤따르는
단위 POS를 함께 증명하는 typed path다. `5천톤`, `6백미터`처럼 ASCII 숫자 뒤 수사와 일반
단위명사가 이어지는 구조는 별도 경계로 유지한다. `서사극이라`, `인쇄업자가`처럼 구조적으로
증명하지 못한 조사 host와 의미상 동음이의어는 열지 않는다.
