...
development 명시적 품사 `smart`는 embedded가 TP 443 / FP 2 / FN 57, full-POS가
TP 444 / FP 2 / FN 56이다.
...
현재 처리량과 latency는 [선어말어미 뒤 `-으되` continuation](2026-07-15-eudoe-continuation.md)을
기준으로 한다.
...
- 남은 full-POS `smart` FN 56건은 `boundary-rejected` 42건, `surface-missing` 9건,
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 56건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 444 / FP 2 / FN 56
...
나머지 14건은 report의 원인 이름을 바로 구현 방향으로 해석하지 않고 gold 위치와 source
annotation을 먼저 판정한다.
...
1. 14개 non-boundary case의 gold span, raw lemma·tag와 source annotation을 먼저 감사한다.
...
- development는 FN 56 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
