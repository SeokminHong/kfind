...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| embedded | smart | 419 / 0 / 81 | 100.00% | 83.8% | 91.19% |
| full-POS | smart | 426 / 0 / 74 | 100.00% | 85.2% | 92.01% |
...
development 명시적 품사 `smart`는 embedded가 TP 446 / FP 2 / FN 54, full-POS가
TP 447 / FP 2 / FN 53이다.
...
현재 처리량과 latency는
[상태 용언 현재 평서형 후속 형태 continuation](2026-07-15-descriptive-declarative-continuation.md)을
기준으로 한다.

품사를 생략하는 사람용 1,000-case fixture에서 full-POS `smart`는 TP 421, FP 0, FN 79,
precision 100.00%, recall 84.2%, F1 91.42%다. embedded `smart`는 TP 322, FP 0, FN 178이다.
embedded `smart`는 기대 품사를 plan에 포함하는
비율이 48.4%이므로 사람용 기본 경로를 대신하지 않는다.

explicit-POS test fixture의 품사를 제거한 User persona도 full-POS `smart`에서 TP 421, FP 0,
FN 79, precision 100.00%, recall 84.2%, F1 91.42%다. `이다 -> 매일`은 bounded 지정사 관형형
...
- 명사류 뒤의 `이면/면`은 접속 조사다. 받침 있는 말은 `이면`, 받침 없는 말은 `면`을 쓰며
  뒤에 다른 조사를 잇지 않는다.
- 남은 full-POS `smart` FN 53건은 `boundary-rejected` 41건, `surface-missing` 9건,
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 53건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 447 / FP 2 / FN 53
- explicit-POS test full-POS `smart`: TP 426 / FP 0 / FN 74
- Agent embedded `any`: TP 482 / FP 11 / FN 18
- Human full-POS `smart`: TP 421 / FP 0 / FN 79
- hard-negative: 22건, 기존 FP 4건

| 원인 | 품사별 건수 | 합계 |
| --- | --- | ---: |
| `boundary-rejected` | adjective 6, determiner 4, noun 6, numeral 6, pronoun 6, verb 13 | 41 |
| `surface-missing` | adjective 4, noun 2, numeral 1, verb 2 | 9 |
| `span-mismatch` | verb 1 | 1 |
| `lexicon-missing` | adjective 2 | 2 |

`boundary-rejected` 41건은 다음 고정 집합이다. predicate 19건의 gold surface와 any-boundary
rule path는 [제한된 사전 표면형 계층](2026-07-15-dictionary-surface-lexicon.md)의 source
report에서 확인한다.

| 품사 | 쿼리 |
| --- | --- |
| adjective | `안되다`, `좋다`, `없다`, `비싸다`, `있다`, `크다` |
| determiner | `사`, `이`, `두`, `열` |
...
- development는 FN 53 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
  4건 외 신규 FP는 0이어야 한다. 통과 후보가 여럿이면 FN이 적은 후보를 먼저 선택하고 FN이
  같을 때 FP가 적은 후보를 선택한다.
- 규칙 고정 뒤 explicit-POS test는 FN 74 이하, precision 99.00% 이상이어야 한다. Agent와 Human도
