...
[구조 기반 어미 목록 전환 보고서](2026-07-16-structural-ending-catalog.md)에서 최신
`origin/main` 대비 고정 품질과 morphology·CLI 성능을 검증했다. full-POS 명시 품사 품질과
hard-negative gate는 유지했고, 사람용 무품사 F1과 실제 CLI 처리량·RSS는 개선됐다.

후속 작업은 `smart` case microbenchmark에서 구조 evidence를 구성하는 비용을 줄이는 것이다.
embedded `smart`의 1개 TP 감소와 붙여 쓴 수량·단위, fused 보조용언 span을 별도 slice로
다루되, 의미상 동음이의어 분류나 표면형 예외 목록은 추가하지 않는다.
