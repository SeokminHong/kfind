...
development 명시적 품사 `smart`는 embedded TP 436 / FP 2 / FN 64, full-POS TP 440 / FP 2 /
FN 60이다. full-POS `smart`가 embedded보다 test에서 추가로 찾는 5건은 모두 명사다. `token`과 `any`에서는 두
lexicon profile의 품질이 같다. 세부 품사와 품질 계약은
[User smart precision 품질·성능](2026-07-14-user-smart-precision.md), 현재 처리량과 latency는
[ㅎ 불규칙 core lexicon recall](2026-07-14-h-irregular-recall.md)을 기준으로 한다.
...
- `ending.connective-ji` left-edge의 bounded token 판정은 `없다/VA -> 없지는`과 같은 candidate
  ...
  오른쪽 token 경계를 유지한다.
- 남은 full-POS `smart` FN 60건은 `boundary-rejected` 43건, `surface-missing` 11건,
...
1. development full-POS `smart`의 TP 440 / FP 2 / FN 60을 기준선으로 사용한다. 반복되는 noun
   `particle.subject` 세 건 `서사극이라`, `인쇄업자가`, `197명이`를 다음 후보군으로 조사한다.
2. 세 candidate는 오른쪽 continuation, 명사 compound, 수사 prefix라는 경계 조건이 다르다.
   위치 유형별로 같은 candidate surface hard-negative를 먼저 고정하고 compact component의 exact
   path가 positive와 negative를 구분하는지 측정한다. 구분되지 않는 위치 유형은 제품에 열지 않는다.
3. development에서 FN 60 미만, precision 99.00% 이상과 기존 hard-negative 신규 FP 0을 모두
