...
1. 지정사 필터링의 정책 이름, 기본값, resource 오류, 상한 초과, JSON/explain 계약을 스펙에
   먼저 정의한다.
2. 결과를 보기 전에 기존 corpus와 NFC 문장 hash가 겹치지 않는 unseen source와 fixture를
   고정한다.
3. dev에서 정한 비용 정책을 unseen source로 검증한다. recall 80.00%, precision 99.00%와
   hard-negative 신규 FP 0을 함께 만족할 때만 지정사 결과 필터링을 제품 범위로 올린다.
4. `-기` 명사형 뒤 조사 continuation은 지정사 필터링과 분리된 규칙 단위로 다룬다.
