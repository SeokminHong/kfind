...
- 최종 unseen 입력은 UD Korean-PUD r2.18 test다. source-only 선택 계약은 양성 436개,
  ...
  `3a6310ea65aa0fd43af90f77dc2ab9208cff4d4d544cce2a444c006ee64f35bc`를 고정한다.
- PUD source와 fixture는 아직 manifest·Docker 산출물에 없고 backend 결과도 관찰하지 않았다.
...
1. `sources.json`과 PUD source adapter를 추가해 921개 fixture를 생성한다. backend를
   실행하기 전에 source digest, case 수, excluded 22개, 중복 0개와 expected fixture
   digest가 일치해야 한다. adapter는 `UPOS=AUX`, `XPOS=VC`, `DEPREL=cop`를 함께 확인하고
   lemma `_`인 22개가 음성으로 들어가지 않게 한다.
2. benchmark에 `copula-lattice` 제품 후보 투영을 추가한다. `accept`·`ambiguous`·
...
3. 밀봉된 PUD fixture를 한 번 평가한다. gold recall 80.00%, target precision 99.00%,
   `unresolved` 0개, revised hard-negative 신규 FP 0개와 기존 품질 gate를 모두 통과하면
   CLI·Rust·WASM에 opt-in 정책을 구현한다. 실패하면 제품 옵션을 추가하지 않는다.
4. `-기` 명사형 뒤 조사 continuation은 지정사 판정 작업을 닫은 뒤 별도 규칙
   단위로 다룬다.
