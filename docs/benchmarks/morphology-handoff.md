...
[계약 보정 지표와 구조 판정 품질](2026-07-16-contract-adjusted-structural-quality.md)에서
최신 `origin/main`과 test·development·Agent·Human strict 품질이 같은 것을 확인했다. 공통
hard-negative 24건의 FPᶜ는 3건에서 1건으로 줄었고, 추가 guard 2건도 TN이다. morphology
microbenchmark와 100 MiB CLI의 모든 변화는 10% 경고선 안이다.

후속 작업은 development FN 48건을 primary cause별로 다시 분류해 가장 큰 일반 규칙 하나를
선택하는 것이다. 남은 FPᶜ인 `주지 스님 → v:주다`는 정상 `V+EC N` recall을 해치지 않는
구조 proof가 있을 때만 제거한다. 의미상 동음이의어 분류나 표면형 예외 목록은 추가하지 않는다.
