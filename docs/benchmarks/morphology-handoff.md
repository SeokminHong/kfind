...
- `smart`의 명사·대명사·수사·관형사 branch는 문자열 token 경계 또는 compact component
  resource의 같은 fine POS를 가진 완전한 형태 component 근거가 있어야 한다. component 경계를
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| embedded | smart | 429 / 0 / 71 | 100.00% | 85.8% | 92.36% |
| full-POS | smart | 436 / 0 / 64 | 100.00% | 87.2% | 93.16% |
...
development 명시적 품사 `smart`는 embedded가 TP 459 / FP 2 / FN 41, full-POS가
TP 460 / FP 2 / FN 40이다.
...
precision 100.00%, recall 86.2%, F1 92.59%다. embedded `smart`는 TP 322, FP 0, FN 178이다.
embedded `smart`는 기대 품사를 plan에 포함하는
비율이 48.4%이므로 사람용 기본 경로를 대신하지 않는다.

explicit-POS test fixture의 품사를 제거한 User persona도 full-POS `smart`에서 TP 431, FP 0,
FN 69, precision 100.00%, recall 86.2%, F1 92.59%다. `이다 -> 매일`은 bounded 지정사 관형형
...
- `smart` component는 명사·대명사·수사·관형사의 exact component span만 복구한다.
...
- 남은 full-POS `smart` FN 40건은 `boundary-rejected` 28건, `surface-missing` 8건,
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 40건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 460 / FP 2 / FN 40
- explicit-POS test full-POS `smart`: TP 436 / FP 0 / FN 64
- Agent embedded `any`: TP 482 / FP 11 / FN 18
- Human full-POS `smart`: TP 431 / FP 0 / FN 69
- hard-negative: 22건, 기존 FP 4건

| 원인 | 품사별 건수 | 합계 |
| --- | --- | ---: |
| `boundary-rejected` | adjective 6, determiner 2, noun 6, numeral 2, verb 12 | 28 |
| `surface-missing` | adjective 4, noun 2, verb 2 | 8 |
| `span-mismatch` | numeral 1, verb 1 | 2 |
| `lexicon-missing` | adjective 2 | 2 |

`boundary-rejected` 28건은 다음 고정 집합이다. predicate 18건의 gold surface와 any-boundary
...
3. predicate 18건은 반복되는 rule path부터 대조한다. `ending.connective-ji` left-edge 3건은
   현재 대조군을 구분하지 못하므로 다시 열지 않는다. 남은 명사·수사·관형사 10건은 이번 exact
   component 판정에서 거부된 유형이므로 whole-token 분석보다 substring을 우선하지 않는다.
4. 하나의 일반 규칙 또는 하나의 사전 생성 계약을 한 작업 단위로 구현한다. 개발 fixture와
   hard-negative를 통과한 뒤에만 고정 test, Agent, Human과 성능을 측정한다.

### 채택 조건과 중단 조건

- development는 FN 40 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
  4건 외 신규 FP는 0이어야 한다. 통과 후보가 여럿이면 FN이 적은 후보를 먼저 선택하고 FN이
  같을 때 FP가 적은 후보를 선택한다.
- 규칙 고정 뒤 explicit-POS test는 FN 64 이하, precision 99.00% 이상이어야 한다. Agent와 Human도
