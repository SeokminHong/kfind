...
development 명시적 품사 `smart`는 embedded가 TP 445 / FP 2 / FN 55, full-POS가
TP 446 / FP 2 / FN 54다.
...
현재 처리량과 latency는
[접속 조사 `이면/면`의 명사류 결합](2026-07-15-connector-myeon-particle.md)을
기준으로 한다.
...
- 명사류 뒤의 `이면/면`은 접속 조사다. 받침 있는 말은 `이면`, 받침 없는 말은 `면`을 쓰며
  뒤에 다른 조사를 잇지 않는다.
- 남은 full-POS `smart` FN 54건은 `boundary-rejected` 42건, `surface-missing` 9건,
  `span-mismatch` 1건, `lexicon-missing` 2건이다.
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 54건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 446 / FP 2 / FN 54
...
나머지 12건은 report의 원인 이름을 바로 구현 방향으로 해석하지 않고 gold 위치와 source
annotation을 먼저 판정한다.
...
1. 12개 non-boundary case의 gold span, raw lemma·tag와 source annotation을 먼저 감사한다.
...
- development는 FN 54 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
