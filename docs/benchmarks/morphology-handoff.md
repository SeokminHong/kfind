...
| full-POS | smart | 421 / 0 / 79 | 100.00% | 84.2% | 91.42% |
| embedded | token | 355 / 0 / 145 | 100.00% | 71.0% | 83.04% |
| full-POS | token | 361 / 0 / 139 | 100.00% | 72.2% | 83.86% |
| embedded | any | 480 / 11 / 20 | 97.76% | 96.0% | 96.87% |
| full-POS | any | 489 / 11 / 11 | 97.80% | 97.8% | 97.80% |
...
현재 처리량과 latency는
[`매일` 인접 문맥 판별 품질·성능](2026-07-14-contextual-maeil-disambiguation.md)을 기준으로 한다.

품사를 생략하는 사람용 1,000-case fixture에서 full-POS `smart`는 TP 417, FP 0, FN 83,
precision 100.00%, recall 83.4%, F1 90.95%다. embedded `smart`는 TP 321, FP 0, FN 179이다.
embedded `smart`는 기대 품사를 plan에 포함하는
비율이 48.4%이므로 사람용 기본 경로를 대신하지 않는다.

explicit-POS test fixture의 품사를 제거한 User persona도 full-POS `smart`에서 TP 417, FP 0,
FN 83, precision 100.00%, recall 83.4%, F1 90.95%다. `이다 -> 매일`은 bounded 지정사 관형형
...
3. kfind 1.0의 안정 Rust API는 `Engine`, `Matcher`, compile option과 오류를 중심으로 확정한다.
   ...
   명시한다.
4. release candidate revision에서 모든 fuzz target을 문서화한 고정 실행 예산으로 실제 실행한다.
   phrase join의 반복 span·큰 gap, 손상된 UTF-8과 component resource 누락을 seed corpus에 포함하고
   crash, panic, hang과 비정상 메모리 증가가 없어야 한다. CI의 compile-only fuzz check는 이
   실행을 대신하지 않는다.
...
3. development에서 FN 58 미만, precision 99.00% 이상과 기존 hard-negative 신규 FP 0을 모두
   ...
   후보를 선택한다.
4. 규칙 고정 뒤 explicit-POS test의 FN 79를 늘리지 않고 precision 99.00% 이상을 유지해야 한다.
