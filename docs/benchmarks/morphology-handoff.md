...
- `ending.connective-ji` left-edge의 bounded token 판정은 `없다/VA -> 없지는`과 같은 candidate
  ...
  오른쪽 token 경계를 유지한다.
- 남은 `surface-missing`에는 core lexicon에서 불규칙 class가 빠진 `어떻다 -> 어떤`,
  `이렇다 -> 이런`, `커다랗다 -> 커다란`이 반복된다. 기존 `DropH` generator를 재사용하고
  검증된 표제어만 core 예외 계층에 추가한다.
...
1. development full-POS `smart`의 TP 437 / FP 2 / FN 63을 기준선으로 사용한다. core lexicon에
   `어떻다`, `이렇다`, `커다랗다`의 `DropH` 분석을 추가하고 각각 `어떤`, `이런`, `커다란`을
   회귀 fixture로 고정한다. 규칙형 `어떻은`, `이렇은`, `커다랗은`은 no-match로 고정한다.
2. 기존 `DropH` generator와 rule data를 재사용한다. 새 lemma별 생성 분기나 full-POS 전체
   표제어의 철자 추정은 추가하지 않는다.
