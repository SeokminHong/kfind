...
현재 처리량과 latency는 [제한된 사전 표면형 계층](2026-07-15-dictionary-surface-lexicon.md)을
기준으로 한다.
...
- `ending.connective-ji` left-edge의 bounded token 판정은 `없다/VA -> 없지는`과 같은 candidate
  ...
  오른쪽 token 경계를 유지한다.
- 남은 full-POS `smart` FN 57건은 `boundary-rejected` 42건, `surface-missing` 9건,
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 57건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 443 / FP 2 / FN 57
...
| `span-mismatch` | numeral 1, verb 3 | 4 |
...
나머지 15건은 report의 원인 이름을 바로 구현 방향으로 해석하지 않고 gold 위치와 source
annotation을 먼저 판정한다.
...
| `span-mismatch` | `치르다/verb`: gold `치렀으되`, 현재 match `치르지` | gold 위치의 any-boundary rule path를 기준으로 boundary 실패를 재분류한다. |
| `span-mismatch` | `쓰다/verb`: gold `쓴다고`, 현재 match `쓰는` | gold 위치의 any-boundary rule path를 기준으로 boundary 실패를 재분류한다. |

### 작업 순서

1. 15개 non-boundary case의 gold span, raw lemma·tag와 source annotation을 먼저 감사한다.
...
- development는 FN 57 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
