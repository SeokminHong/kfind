...
Matrix full-POS의 `이다` FN은 canonical 범위 밖인 `겁니다` 2건과 `이예요` 1건만 남았다.
다음 큰 standard-form FN 묶음은 `오다` 5건, `지다` 4건, `있다` 동사 3건이다. 다음 작업은
surface를 개별 추가하기 전에 이들의 공통 cause와 source component coverage를 다시 분류한다.
