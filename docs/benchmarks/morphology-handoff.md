...
- full-POS가 로드된 `smart` 동사·형용사 branch는 predicate verifier가 활용·continuation을
  소비한 뒤 같은 fine POS의 어간 component가 최저 비용 경로에 있을 때 문자열 왼쪽 경계를
  복구한다. embedded `smart` 용언과 지정사 `PredicateLexical` 계약은 바꾸지 않는다.
...
| full-POS | smart | 456 / 0 / 44 | 100.00% | 91.2% | 95.40% |
...
품사를 생략하는 사람용 1,000-case fixture에서 full-POS `smart`는 TP 451, FP 0, FN 49,
precision 100.00%, recall 90.2%, F1 94.85%다. embedded `smart`는 TP 322, FP 0, FN 178이다.
embedded `smart`는 기대 품사를 plan에 포함하는
비율이 48.4%이므로 사람용 기본 경로를 대신하지 않는다.

explicit-POS test fixture의 품사를 제거한 User persona도 full-POS `smart`에서 TP 451, FP 0,
FN 49, precision 100.00%, recall 90.2%, F1 94.85%다. `이다 -> 매일`은 bounded 지정사 관형형
...
- `smart` component는 명사·대명사·수사·관형사와 full-POS 일반 동사·형용사의 exact component
  span만 복구한다. 용언은 활용·continuation을 소비한 어간 core의 fine POS를 검증한다.
...
- 남은 full-POS `smart` FN 30건은 `boundary-rejected` 17건, `surface-missing` 8건,
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 30건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 470 / FP 2 / FN 30
- explicit-POS test full-POS `smart`: TP 456 / FP 0 / FN 44
- Agent embedded `any`: TP 482 / FP 11 / FN 18
- Human full-POS `smart`: TP 451 / FP 0 / FN 49
- hard-negative: 22건, 기존 FP 4건

| 원인 | 품사별 건수 | 합계 |
| --- | --- | ---: |
| `boundary-rejected` | adjective 3, determiner 2, noun 6, numeral 2, verb 4 | 17 |
| `surface-missing` | adjective 4, noun 2, verb 2 | 8 |
| `span-mismatch` | numeral 1, verb 2 | 3 |
| `lexicon-missing` | adjective 2 | 2 |

`boundary-rejected` 17건은 다음 고정 집합이다. predicate 7건의 gold surface와 any-boundary
...
3. 남은 predicate 7건은 exact component가 거부한 경로와 gold span을 대조한다. 남은
   명사·수사·관형사 10건도 whole-token 분석보다 substring을 우선하지 않는다.
4. 하나의 일반 규칙 또는 하나의 사전 생성 계약을 한 작업 단위로 구현한다. 개발 fixture와
   hard-negative를 통과한 뒤에만 고정 test, Agent, Human과 성능을 측정한다.

### 채택 조건과 중단 조건

- development는 FN 30 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
  4건 외 신규 FP는 0이어야 한다. 통과 후보가 여럿이면 FN이 적은 후보를 먼저 선택하고 FN이
  같을 때 FP가 적은 후보를 선택한다.
- 규칙 고정 뒤 explicit-POS test는 FN 44 이하, precision 99.00% 이상이어야 한다. Agent와 Human도
