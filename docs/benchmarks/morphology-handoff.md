...
development 명시적 품사 `smart`는 embedded가 TP 447 / FP 2 / FN 53, full-POS가
TP 448 / FP 2 / FN 52다.
...
- 남은 full-POS `smart` FN 52건은 `boundary-rejected` 40건, `surface-missing` 9건,
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 52건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 448 / FP 2 / FN 52
...
| 원인 | 품사별 건수 | 합계 |
| --- | --- | ---: |
| `boundary-rejected` | adjective 6, determiner 4, noun 6, numeral 6, pronoun 6, verb 12 | 40 |
| `surface-missing` | adjective 4, noun 2, numeral 1, verb 2 | 9 |
| `span-mismatch` | verb 1 | 1 |
| `lexicon-missing` | adjective 2 | 2 |

`boundary-rejected` 40건은 다음 고정 집합이다. predicate 18건의 gold surface와 any-boundary
...
| verb | `버리다`, `이루다`, `낚다`, `타다`, `비추다`, `빼다`, `지나다`, `지다`, `내다`, `소리치다`, `돌아가다`, `가르치다` |
...
3. predicate 18건은 반복되는 rule path부터 대조한다. `ending.connective-ji` left-edge 3건은
...
- development는 FN 52 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
