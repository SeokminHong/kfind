...
- [파생 용언 앞 명사 파생 경로 recall](2026-07-18-nominal-derivational-predicate.md)
- [소실 지정사 축약 recall](2026-07-18-copula-lost-span.md)
...
- 필요한 compact resource의 누락·손상·schema·source·package version mismatch는 초기화
  오류다. 문자열 경계 판정으로 fallback하지 않는다.
...
bytes에서 version field 32 bytes가 추가된 37,103,813 bytes다. full morphology lattice는 제품
판정을 바꾸지 않는 진단 도구로만 유지한다.

## 검증 게이트

- 고정 test의 TP를 줄이거나 FP를 늘리지 않는다. 명시적으로 분류한 `nonstandard-spacing`은
  strict delta에 남기되 이 표준형 gate에서 제외한다.
...
- 비표준 활용, 오타와 불안정한 띄어쓰기는 canonical 형태 규칙에 합치지 않는다. 별도 opt-in
  robustness 축에서 자연 원문 fixture를 먼저 검증하며, 그전까지 관련 FP/FN을 허용한다.
...
`못`을 구분했다. Canonical과 development 품질은 유지했고 hard-negative 동형어 FP 1건을
제거했다. Test matrix의 유일한 불리한 이동은 `nonstandard-spacing`인 `안나와요` 1건이다.
Schema 5 component는 exact package version을 보존하며 Homebrew `post_install`, 수동
`--check-data`와 npm prepack/release 검증으로 package update와 resource update를 묶는다.
후속 작업은 비표준 붙여쓰기를 이 규칙에 다시 섞지 말고 별도 robust 지원에서 다룬다.
...
cases/s가 됐다. 품질과 전체 test span은 그대로다. 검토된 query matrix와 외부 snapshot의
현재 Agent matrix는 26,839.2 cases/s, Lindera는 24,240.9 cases/s다.
...
[선두 부사 component recall](2026-07-17-leading-adverb-component-recall.md)에서 token 시작의
`MAG` node와 남은 suffix가 완전 graph path를 이룰 때 runtime exact component를 유지했다.
Test matrix embedded·full-POS는 `안` 2건, `드디어`, `오래`를 회수해 `PNᶜ=1,401`에서
FNᶜ가 각각 135→131, 50→46이 됐다. Human은 세 건을 회수해 FNᶜ가 52→49가 됐고,
development의 두 explicit-POS profile은 `많이들→많이` 한 건을 회수했다. Canonical
embedded·full-POS도 두 건씩 회수해 FNᶜ가 53→51, 11→9가 됐다. 모든 FP·FPᶜ case와
hard-negative는 같다. Matrix contract 정의, annotation과 gate는 변경하지 않았다.

남은 test matrix full-POS FNᶜ 46건은 `boundary-rejected` 22건,
`gold-or-adapter` 15건, `surface-missing` 6건, `continuation-rejected` 2건,
`span-mismatch` 1건이다. 남은 부사 2건은 graph가 suffix를 완성하지 못한
...
[전망 종결형 `-(으)리라` recall](2026-07-17-prospective-final-recall.md)에서 표준
`-(으)리라`를 terminal `Eu` continuation으로 추가했다. Test matrix embedded·full-POS·Agent·
Human은 `않으리라→않다`를 회수해 `PNᶜ=1,401`에서 FNᶜ가 각각 130→129, 44→43,
35→34, 47→46이 됐다. 모든 FP·FPᶜ, canonical, development와 hard-negative는 같다.
비표준 `보로→보다`는 `any`에서도 계속 거부한다. Matrix contract 정의, annotation과 gate는
변경하지 않았다.

Raw test matrix full-POS FNᶜ 43건은 `boundary-rejected` 20건,
...
[사전 합의 `-이` 부사형 recall](2026-07-18-dictionary-adverbial-i.md)에서 `-없다`·`-같다`
계열과 `르→ㄹ리`의 제한된 후보를 만들고, 한국어기초사전과 표준국어대사전이 원형 형용사와
결과 부사를 독립 등재한 88개만 기본 `inflection` 표면으로 승격했다. `smart`는 전체 `MAG`
구조와 양쪽 token 경계를 요구한다. Test matrix full-POS는 raw FN 18→13, FNᶜ 14→9이고
FP 4·FPᶜ 0을 유지했다. Canonical과 development도 FP 증가 없이 FN을 각각 1건·3건 줄였다.
잔여 raw FN 13건은 `product-fix 7`, `structural-redesign 2`, `gold-alignment-error 1`,
`nonstandard-input 3`이며 미분류는 0건이다.

다음 독립 PR은 대명사 축약 2건 `무어→무언가`, `누구→누군가가`를 처리한다. 사전의 별도
표제어를 alias로 합치지 않고, 대명사 stem과 `ㄴ가`, 선택적 조사 연쇄를 갖는 bounded 구조로
검증한다.

[대명사 계사·의문 어미 축약 recall](2026-07-18-pronoun-copula-ending.md)에서 query 표제어와
축약 표면의 대응을 선언하고, `smart`가 표면 전체의 `NP + VCP + E+` source 분석과 종단
`EC/EF`를 요구하도록 했다. `무어→무언가`, `누구→누군가가`를 회수해 test matrix full-POS
raw FN 13→11, FNᶜ 9→7이고 FP 4·FPᶜ 0을 유지했다. Canonical embedded·full-POS도 FN을
각각 2건 줄였고 development와 hard-negative는 그대로다. 잔여 raw FN 11건은
`product-fix 5`, `structural-redesign 2`, `gold-alignment-error 1`, `nonstandard-input 3`,
미분류 0건이다.
...
[파생 용언 앞 명사 파생 경로 recall](2026-07-18-nominal-derivational-predicate.md)에서 token
왼쪽 경계부터 `N+ + XSN+`로 끝나는 query core와 이어지는 `XSV/XSA + E+`를 함께
검증했다. `잠식당→잠식당하기`를 회수해 test matrix full-POS raw FN은 7→6, FNᶜ는
3→2가 됐으며 FP 4·FPᶜ 0과 hard-negative `FP 6 / TN 33`을 유지했다. 잔여 raw FN
6건은 `structural-redesign 2`, `gold-alignment-error 1`, `nonstandard-input 3`, 미분류
0건이다.

[소실 지정사 축약 recall](2026-07-18-copula-lost-span.md)에서 선언된 `걸까` anchor가
맞은 뒤 exact source의 `체언+ + VCP + E+`와 종단 `EC/EF`, 정렬된 VCP span 소실을
검증했다. `이다→걸까`를 회수해 test matrix full-POS raw FN은 6→5, FNᶜ는 2→1이
됐으며 FP 4·FPᶜ 0과 hard-negative `FP 6 / TN 33`을 유지했다. 잔여 raw FN 5건은
`structural-redesign 1`, `gold-alignment-error 1`, `nonstandard-input 3`, 미분류 0건이다.

다음 독립 PR은 `하→책임하에서`의 한 음절 명사 source component를 처리한다. 독립 명령형
`그래 네가 가.`의 `가`처럼 query가 token 전체 용언인 경우는 그대로 유지하면서, 명사 query는
정렬된 source component와 완성된 체언·조사 구조로만 내부 한 음절 span을 회수한다.

[명사 결합 접미사 recall](2026-07-18-attached-nominal-suffix.md)에서 고정 NIKL snapshot이
합의한 접미사 어휘와 `체언+ + 접미사/NNG + 조사+` 완전 경로를 함께 검증했다. 후보는 한 번만
생성하며 사전 catalog 생성기와 validator만 분리한다. `하→책임하에서`를 회수해 test matrix
full-POS raw FN은 5→4, FNᶜ는 1→0이 됐고 FP 4·FPᶜ 0과 hard-negative
`FP 6 / TN 33`을 유지했다. 독립 명령형 `그래 네가 가.`의 마지막 `가`는 계속 회수한다.
