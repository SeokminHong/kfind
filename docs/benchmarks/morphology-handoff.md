...
1. Korean-Kaist·KSL dev에서 정상 VCP gold reject 13개와 같은 실패 원인을 분류한다.
2. 지정사 필터링의 정책 이름, 기본값, resource 오류, 상한 초과, JSON/explain 계약을 스펙에
   먼저 정의한다.
3. 결과를 보기 전에 기존 corpus와 NFC 문장 hash가 겹치지 않는 unseen source와 fixture를
   고정한다.
4. dev에서 정한 비용 정책을 unseen source로 검증한다. recall 80.00%, precision 99.00%와
   hard-negative 신규 FP 0을 함께 만족할 때만 지정사 결과 필터링을 제품 범위로 올린다.
5. `-기` 명사형 뒤 조사 continuation은 지정사 필터링과 분리된 규칙 단위로 다룬다.

Korean-GSD 결과에 맞춘 비용·threshold·fixture 가중치 변경은 허용하지 않는다.
