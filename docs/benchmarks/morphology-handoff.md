...
- `ending.past`와 `ending.future` verifier state는 `ending.connective-eudoe`의 `으되`를
  ...
  token만 복구한다.
- 현재 서술형 `-ㄴ다/-는다` branch는 `고`, `는`, `던`, `면`, `니`, `며`, `면서`, `는데`,
  `지`를 token 안에서 소비한다. bare 서술형은 유지하고 `거나`, `든가`, `든지` 같은 종결 어미
  뒤 조사와 두 번째 후속 형태 연쇄는 별도 상태로 남긴다.
...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| embedded | smart | 418 / 0 / 82 | 100.00% | 83.6% | 91.07% |
| full-POS | smart | 425 / 0 / 75 | 100.00% | 85.0% | 91.89% |
| embedded | token | 358 / 0 / 142 | 100.00% | 71.6% | 83.45% |
| full-POS | token | 365 / 0 / 135 | 100.00% | 73.0% | 84.39% |
| embedded | any | 482 / 11 / 18 | 97.77% | 96.4% | 97.08% |
| full-POS | any | 492 / 11 / 8 | 97.81% | 98.4% | 98.11% |

development 명시적 품사 `smart`는 embedded가 TP 444 / FP 2 / FN 56, full-POS가
TP 445 / FP 2 / FN 55다.
...
현재 처리량과 latency는
[현재 서술형 후속 형태 continuation](2026-07-15-present-declarative-continuation.md)을
기준으로 한다.

품사를 생략하는 사람용 1,000-case fixture에서 full-POS `smart`는 TP 420, FP 0, FN 80,
precision 100.00%, recall 84.0%, F1 91.30%다. embedded `smart`는 TP 322, FP 0, FN 178이다.
embedded `smart`는 기대 품사를 plan에 포함하는
비율이 48.4%이므로 사람용 기본 경로를 대신하지 않는다.

explicit-POS test fixture의 품사를 제거한 User persona도 full-POS `smart`에서 TP 420, FP 0,
FN 80, precision 100.00%, recall 84.0%, F1 91.30%다. `이다 -> 매일`은 bounded 지정사 관형형
...
- 현재 서술형 뒤의 `고`, `는`, `던`, `면`, `니`, `며`, `면서`, `는데`, `지`는 완성된
  predicate token의 일부다. `받다 -> 받는다는`, `받들다 -> 받든다는`와
  `영원히 함께한다던 말도`의 `한다던`을 복구하며 `말도`는 다음 명사 token으로 남긴다.
  `쓴다도`, `먹는다도`는 거부한다.
- 남은 full-POS `smart` FN 55건은 `boundary-rejected` 42건, `surface-missing` 9건,
  `span-mismatch` 2건, `lexicon-missing` 2건이다.
...
다음 형태 품질 작업은 명시적 품사 full-POS `smart`의 남은 FN 55건을 줄이는 제품 변경이다.
...
- development full-POS `smart`: TP 445 / FP 2 / FN 55
- explicit-POS test full-POS `smart`: TP 425 / FP 0 / FN 75
- Agent embedded `any`: TP 482 / FP 11 / FN 18
- Human full-POS `smart`: TP 420 / FP 0 / FN 80
- hard-negative: 22건, 기존 FP 4건

| 원인 | 품사별 건수 | 합계 |
| --- | --- | ---: |
| `boundary-rejected` | adjective 7, determiner 4, noun 6, numeral 6, pronoun 6, verb 13 | 42 |
| `surface-missing` | adjective 4, noun 2, numeral 1, verb 2 | 9 |
| `span-mismatch` | numeral 1, verb 1 | 2 |
| `lexicon-missing` | adjective 2 | 2 |

`boundary-rejected` 42건은 다음 고정 집합이다. predicate 20건의 gold surface와 any-boundary
rule path는 [제한된 사전 표면형 계층](2026-07-15-dictionary-surface-lexicon.md)의 source
report에서 확인한다.

| 품사 | 쿼리 |
| --- | --- |
| adjective | `나쁘다`, `안되다`, `좋다`, `없다`, `비싸다`, `있다`, `크다` |
| determiner | `사`, `이`, `두`, `열` |
| noun | `도구`, `서사극`, `건전`, `업자`, `속`, `년대` |
| numeral | `만`, `둘`, `일`, `하나`, `다섯`, `천` |
| pronoun | `아무`, `자기`, `무엇`, `거기`, `그녀`, `여기` |
| verb | `버리다`, `이루다`, `낚다`, `타다`, `비추다`, `빼다`, `지나다`, `지다`, `내다`, `소리치다`, `돌아가다`, `가르치다`, `불다` |
...
| `span-mismatch` | `백/numeral`: gold `백이면`, 현재 match 뒤의 `백` | 다른 출현의 성공과 분리해 gold 위치의 조사 continuation을 판정한다. |

### 작업 순서

1. 13개 non-boundary case의 gold span, raw lemma·tag와 source annotation을 먼저 감사한다.
   `span-mismatch`는 문장 단위 prediction이 아니라 gold 출현별로 판정한다. 명백한 annotation·adapter
   문제는 제품 규칙으로 덮지 않고 진단 결과로 남긴다.
2. `surface-missing`과 `lexicon-missing`은 개별 표면형을 손으로 추가하지 않는다. 고정 NIKL
...
3. `boundary-rejected`는 any-boundary의 core·token span, token 내 위치와 모든 rule path를
...
4. predicate 20건은 반복되는 rule path부터 대조한다. `ending.connective-ji` left-edge 3건은
   현재 대조군을 구분하지 못하므로 다시 열지 않는다. 명사·수사·관형사·대명사 22건은 whole-token
   사전 분석보다 substring을 우선하지 않고, 완전한 component 분석이 있는 유형만 묶는다.
5. 하나의 일반 규칙 또는 하나의 사전 생성 계약을 한 작업 단위로 구현한다. 개발 fixture와
   hard-negative를 통과한 뒤에만 고정 test, Agent, Human과 성능을 측정한다.

### 채택 조건과 중단 조건

- development는 FN 55 미만, precision 99.00% 이상이어야 한다. hard-negative 22건의 기존 FP
  4건 외 신규 FP는 0이어야 한다. 통과 후보가 여럿이면 FN이 적은 후보를 먼저 선택하고 FN이
  같을 때 FP가 적은 후보를 선택한다.
- 규칙 고정 뒤 explicit-POS test는 FN 75 이하, precision 99.00% 이상이어야 한다. Agent와 Human도
