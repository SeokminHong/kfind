...
지정사 결과 필터링은 아직 제품 범위가 아니다. 정상 VCP gold reject 13개의 dev 원인을
분류하고, 정책과 출력 계약을 스펙에 정의한 뒤, 별도 unseen source에서 검증해야 한다. 자세한
순서는 [형태소 검색 개선 핸드오프](morphology-handoff.md)를 따른다.
