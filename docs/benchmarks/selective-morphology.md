...
- 고정 test, dev, hard-negative와 별도 unseen source의 역할을 섞지 않는다.

## 후속 작업

지정사 결과 필터링은 아직 제품 범위가 아니다. dev gold-aligned lattice reject의 구조별 분류는
[지정사 lattice dev gold 진단](2026-07-13-copula-dev-diagnosis.md)을 기준으로 한다. 정책과 출력
계약을 스펙에 정의한 뒤 별도 unseen source에서 검증해야 한다. 자세한 순서는
