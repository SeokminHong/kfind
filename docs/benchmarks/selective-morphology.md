...
지정사 결과 필터링은 아직 제품 범위가 아니다. 후보 `copula-lattice`는 추가 threshold 없이
`reject`만 제거하고 `ambiguous`와 `unresolved`는 유지한다. UD Korean-PUD r2.18의 밀봉된
921-case fixture와 제품 gate를 모두 통과한 뒤에만 opt-in으로 노출한다. 구현 순서는
