...
schema 3은 source 분석과 모든 unknown class를 보존해 1,652개 candidate를 모두 평가한다.
gold target recall은 94.66%지만 non-gold target reject 비율은 24.30%다. source 최저 비용은
...
| --- | ---: | ---: | ---: | ---: |
| gold target | 936 | 886 | 50 | 94.66% |
| non-gold target | 716 | 542 | 174 | 75.70% |

- 1,652개 candidate가 모두 `evaluated`였고 오류와 `ambiguous`는 없었다.
- 전체 판정은 `accept` 1,428개, `reject` 224개다.
...
gold span의 byte 중첩으로 계산했다. 이 기준에서 precision은 62.04%, recall은 94.66%,
specificity는 24.30%다. threshold를 추가하기 전에 별도 blind source에서 비용 분포와
