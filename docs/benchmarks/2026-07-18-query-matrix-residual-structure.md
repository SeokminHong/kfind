...
Strict와 contract-adjusted 결과는 reclassified case가 0건이라 같다. Canonical,
development, hard-negative의 confusion matrix는 모두 유지됐다.

잔여 FNc 18건은 기존 처분 장부와 다시 대조해 미분류 0건을 확인했다. 잔여 FPc 4건은
동형어 의미 선택 또는 source lemma/POS 정렬 문제라 현재의 bounded morphology 구조만으로
더 줄일 수 없다. 세 국립국어원 사전의 exact 표제어·품사·관계에는 새 alias나 품사 record를
제품에 추가할 근거가 없었다.
...
| 처분 | 건수 |
| --- | ---: |
| structurally-unresolvable | 1 |
| gold-or-adapter | 7 |
| out-of-contract | 9 |
| cost-prohibitive | 1 |
| dictionary-required | 0 |

Raw FNc는 18건, 미분류 FNc는 0건이다. 기존 장부의 case identity, failure cause와 사전
snapshot hash가 모두 후보 report와 일치했다. 제품 규칙이나 fixture annotation으로 재분류하지
않았다.
