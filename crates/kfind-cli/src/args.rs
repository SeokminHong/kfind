...
#[command(
    ...
    group(ArgGroup::new("integration_mode").args(["init", "uninstall"]))
)]
pub struct Args {
    /// Korean lemma, `|` alternatives, short phrase, or tagged query.
    #[arg(required_unless_present_any = ["init", "uninstall", "check_data", "agent_hook"])]
    ...
    /// Select an integration target. Repeat to manage multiple agents.
    #[arg(
        long,
        value_enum,
        action = ArgAction::Append,
        requires = "integration_mode",
