...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() < 6 {
        return Err(
            "usage: kfind-data-build-morphology OUTPUT SOURCE_SHA256 MATRIX CHAR_DEF UNK_DEF CSV..."
                .into(),
        );
    }
    ...
    let decoded =
        decode_morphology_resource(&output.display().to_string(), &resource, &source_digest)?;
