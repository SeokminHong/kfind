...
Project hooks run after each agent's trust review. In Codex, inspect and trust them with `/hooks`. The `SessionStart` hook adds an instruction to use kfind for every Korean source-code and documentation search, even when the skill is not selected automatically. The pre-tool hook blocks `rg`, `grep` variants, and `git grep` when their search pattern contains Korean text, then directs the agent to kfind.
