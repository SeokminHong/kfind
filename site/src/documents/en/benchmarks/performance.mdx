...
Fast anchor scanning does not imply that complete morphological searches run as fast as ripgrep. kfind also pays for query compilation, resource initialization, and candidate verification. Read execution time together with search quality in the [search-strategy comparison](/benchmarks#search-strategy-baseline).
