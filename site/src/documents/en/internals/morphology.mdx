...
- The complete token has an exact `predicate + EC + VX + E+` analysis.
- The suffix belongs to the resultative `-아/어지다` family.
...
`smart` decodes only bounded tokens around an anchor from the compact [component resource](/reference/glossary#component-resource). A `StructuralConstraint` compares the POS sequence, component spans, continuations, and neighboring-token conditions required by the query. It uses only observable morphology and does not interpret meaning.
