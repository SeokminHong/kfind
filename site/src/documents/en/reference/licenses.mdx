...
kfind source code uses the MIT License in the repository’s `LICENSE`. It covers the Rust crates, site source, and npm wrapper.
...
The repository and GitHub release source include `LICENSES.md` and the NIKL-derived-data notice. The npm package carries the same notice in `LICENSES.md` and `assets/LICENSES`. Homebrew installs it with the package documentation.
