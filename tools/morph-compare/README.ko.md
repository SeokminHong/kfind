...
Query matrix의 raw FN disposition 장부는 report의 fixture hash, backend, case ID, 질의,
품사, gold byte span과 failure cause가 모두 일치해야 한다. 다음 명령은 현재 raw FN이 빠짐없이
한 번씩 분류됐는지 검증한다. 별도 contract registry가 구현 목표 확인, 기대값 교정과 비표준
입력 제외를 기록한다.
...
추가한다. kfind 표와 차트는 raw와 contract를 나란히 표시한다. 비표준 입력만 계약 모수에서
제외하며 미구현 표준 문법은 FNᶜ로 유지한다. `kfind` 프로필별 버전·artifact SHA-256와 full-POS에서
