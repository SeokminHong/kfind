...
median과 min/max를 기록한다. explicit-POS 비교는 현재 Agent 측정과 외부 분석기 4종의
고정 품질·성능 스냅샷을 같은 표에 둔다. 전체 test 보고서는 두
...
`product-external-comparison.svg`는 같은 explicit-POS task에서 Agent, Kiwi, Lindera,
MeCab-ko, KOMORAN의 precision·recall·F1, 초기화, 처리량, p95와 peak RSS를 비교한다.
