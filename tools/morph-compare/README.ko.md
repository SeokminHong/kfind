...
Kiwi·Lindera·MeCab-ko·KOMORAN의 고정 품질 스냅샷과 비교한다. 외부 분석기와 corpus는
...
test fixture나 고정한 외부 도구·어댑터 설정을 바꿀 때만 외부 스냅샷을 명시적으로 갱신한다.
기본 벤치마크는 fixture 또는 schema가 맞지 않으면 갱신 명령을 안내하고 실패한다.
...
median과 min/max를 기록한다. 외부 분석기 품질 스냅샷은 현재 성능 표에 넣지 않는다. 전체 test 보고서는 두
