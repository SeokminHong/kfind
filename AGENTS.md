...
- 저장소의 README, 기술 사양, 도구 안내와 배포 문서는 한국어로 작성한다. 코드 식별자,
  명령, API 이름과 고유 명칭은 원문 표기를 허용한다. 웹사이트는 한국어 원문과 영어 번역을
  함께 제공하며, 이 번역본만 한국어 작성 원칙의 예외다.
- README, 기술 사양, 웹 문서와 도구 안내는 현재 제품의 구조·동작·제약만 설명한다. 변경 전
  상태, 개선 서사, 날짜·커밋별 증감과 완료 이력은 적지 않는다. 해당 문서를 처음 읽는 독자가
  현재 상태만으로 내용을 이해할 수 있어야 한다.
- 날짜·Git revision·baseline/candidate 비교, 실험 결과와 개발 결정의 근거는
  `docs/benchmarks`의 날짜별 보고서에 보존한다. 비측정 개발 결정은
  `docs/decisions`에 기록한다. 현재 문서에서 필요한 결론만 남기고, 이력은 해당 기록으로
  옮긴다.
- 웹 문서의 헤딩은 다루는 대상을 나타내는 명사구로 작성한다. 완결된 문장, 홍보 문구와
  행동을 권하는 문장을 헤딩으로 사용하지 않는다.
...
- 품질 benchmark는 제품과 외부 분석기를 포함한 모든 행에 raw와 contract-adjusted confusion
  matrix, precision, recall과 F1을 함께 기록한다. Contract review가 없는 fixture에서도
  contract-adjusted 값을 생략하지 않으며, raw와 같다는 사실과 review 0건을 명시한다. 성능
  benchmark에는 품질 보정을 적용하지 않고 workload별 단위를 유지한다.
