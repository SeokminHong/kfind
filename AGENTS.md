...
- 사용자가 PR·머지를 요청했거나 처음부터 제품 변경을 `main`까지 반영하는 요청임이
  명백하고, 검증 뒤에도 변경 필요성이 유지되는 경우에만 PR을 생성하고 머지까지 완료한다.
- 분석, 조사, 리뷰, benchmark, prototype과 보고 요청은 push·PR·머지 권한이 아니다. 결과가
  변경 불필요 또는 미채택이면 실험용 코드와 측정 장치를 머지하지 않는다. 결과와 정리 범위를
  먼저 보고하고 사용자가 보존·반영을 명시적으로 승인한 경우에만 원격 반영을 진행한다.
