...
사용한다. embedded 프로필의 false negative는 다음 우선순위로 하나의
`primary_cause`를 갖는다.
...
분류 증거와 primary cause는 JSON failure record에 저장하고, 분류를 위한 추가 컴파일·
검색 비용은 backend 성능에 포함하지 않는다.
