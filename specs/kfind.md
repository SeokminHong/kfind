...
- `--embedded`는 full POS lexicon을 resolve하거나 읽지 않는다. compact component resource의
  로드 여부는 기존 `smart` query plan 정책을 유지한다. `--explain-query`는 full POS 상태를
  `not required (embedded mode)`로 출력한다.
...
- 문자열의 좌우 경계를 판정하는 `boundary`는 제품 정책 축이다. 가능한 지정사 분석을 선택하는
  `copula-lattice`는 benchmark shadow 후보이며 공개 제품 정책 축이 아니다.
...
- `학생일`, `책일`은 사전 표제어가 아니라 각각 체언 host와 VCP 관형형 표면 `일`의 결합을
  검증하는 어절 fixture다.
- 지정사 필터링 후보 `copula-lattice`는 `smart` VCP branch의 `EojeolLattice`에만 적용하는
  benchmark 투영이다. 제품 검색은 `union`으로 고정한다.
...
- 투영은 기존 `morphology-component-compact.kfc`의 include·exclude 비용을 사용하고 판정과
  경로를 benchmark 보고서에만 보존한다. 제품 CLI·Rust library·WASM에는 `disambiguation`이나
  `copula-lattice` 옵션을 노출하지 않는다.
...
metadata hash도 명시적 품사 fixture와 분리해 기록한다.
...
형태 품질 변경은 dev recall 80.00% 이상과 precision 99.00% 이상을 동시에 유지해야 한다.
고정 test fixture는 규칙 선택에 사용하지 않고 회귀만 기록한다. 최종 품질 주장은
...
2026-07-13 밀봉 평가에서 embedded와 full-POS 투영은 모두 TP 285, FP 0, TN 485, FN 151,
target precision 100.00%, gold recall 65.37%, unresolved 0개였다. recall gate 실패로
`copula-lattice`를 공개 제품 정책으로 추가하지 않는다. 이 fixture로 재평가하거나 결과에 맞춰
비용, threshold 또는 fixture를 변경하지 않는다. 측정 계약과 report digest는
[`2026-07-13 지정사 lattice unseen 제품 판정`](../docs/benchmarks/2026-07-13-copula-unseen-evaluation.md)에
고정한다.
