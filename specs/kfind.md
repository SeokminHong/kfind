...
생성기는 surface-only 행 수를 통계로 기록하고 배포 `predicates.tsv`가 64 KiB를 넘으면 실패한다.
행 수는 UTF-8 길이가 다른 활용형·관계형의 실제 parse·배포 비용을 대표하지 않으므로 별도 hard
limit으로 쓰지 않는다. Source snapshot 갱신으로 byte 한도를 넘으면 중복 생성 규칙과 분류 누락을
먼저 해소하고, 한도 변경은 별도 성능·배포 크기 검토로 결정한다. report에는 생략된 생성형, 배포
surface-only 활용형·파생형, source record ID와 artifact byte 수를 구분해 기록한다.
...
각 일치의 raw·NFC span mapping은 다시 계산하며, 준비 context cache는 256개로 제한한다.
