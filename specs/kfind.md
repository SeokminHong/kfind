...
- `EojeolLattice`는 benchmark shadow 계측 전용이다. 지정사 검색은 생성 가능한 분석을 모두
  인정하는 homonym union을 유지하며 corpus-side 분석으로 결과를 필터링하지 않는다.
...
- `학생일`, `책일`은 사전 표제어가 아니라 각각 체언 host와 VCP 관형형 표면 `일`의 결합을
  검증하는 어절 fixture다.
- 지정사 결과 필터링을 제품에 추가하려면 정책 이름, 기본값, resource 오류, 상한 초과와
  JSON/explain 출력을 먼저 이 스펙에 확정하고 별도 unseen source에서 검증한다.
...
baseline으로만 사용한다. 비용·threshold·fixture 가중치를 변경한 구현은 별도 unseen source에서
검증해야 하며, 이 report만으로 union 검색 결과나 기본 정책을 변경하지 않는다.
