...
- gold corpus에 포함된 현재 평서형 `-ㄴ다/는다`와 상태 용언의 `-다`, 회상 관형형 `-던`, 양보 연결형 `-더라도`, 과거 관형 연쇄 `-았/었을`, 과거 의문 종결 연쇄 `-았/었느냐`, `-았/었느냐는`, 이유 연결형 `-(으)니`, 인용 연결형 `-다고`, 현재 서술형 인용·회상·조건 연쇄, 전망 인용 연쇄 `-(으)리라고`, 의도 연결형 `-(으)려고`, 상태 변화 보조 용언 `-아/어지다`, 진행 방향 보조 용언 `-아/어가고`, `-아/어가야`도 v0.1의 제한된 continuation vocabulary에 포함한다.
...
- `LexicalContext`는 `smart` modifier의 품사 전체에 부여하지 않는다. compiler의 typed context
  ...
  등록되지 않은 `빨리`와 `매우` 같은 일반 부사는 component resource 없이 컴파일한다.
...
- corpus resource는 source가 명시한 whole-token 분석과 component 분해, 런타임 조합 경로, unknown 경로와 인접 token 관계를 구분한 `TokenAnalysisGraph`를 제공한다. component span·POS와 정규화된 expression relation은 graph provenance에서 복구 가능해야 하며 raw `analysis_type`은 structural identity에 포함하지 않는다.
- resolver는 query 제약을 만족하는 모든 분석을 `SupportedAnalysisSet`과 proof로 반환하고 `Contradicted`, `Ambiguous`, `Unavailable`을 별도 결과로 보존한다. structural resolver와 proof 순서는 connection cost와 word cost를 사용하지 않으며 query와 무관한 다른 품사·분해 경로는 그 자체로 모순이 아니고 span 또는 component 노출 관계를 구조적으로 정할 수 없을 때만 `Ambiguous`를 반환한다.
...
- structural graph schema 4는 schema 2의 lexical identity, POS, relation projection과 source expression·multi-POS row에서 파생한 categorical morphotactic transition만 보존하고 raw `analysis_type`, dense connection matrix, left/right context ID와 word cost를 저장하지 않는다. scoring projection 감사는 schema 2와 full morphology resource 사이에서 build 전에 완료하며 전체 constraint resolver는 schema 4만 사용하고 scoring schema로 fallback하지 않는다.
- `QueryMorphPattern`은 한 query 분석의 lexical identity, fine POS, core·anchor·consumed·surrounding token span 관계, continuation DFA, 인접 token 제약과 whole/source/runtime component capability만 선언한다. lexical identity는 query 분석에서 나온 한 표제어이며 corpus별 surface 목록이 아니다. 같은 branch의 pattern 합집합은 하나의 resolver 입력으로 평가하고 surface 목록, 비용 임계값과 fallback 순서를 포함하지 않는다.
- nominal reference candidate는 조사 0개의 `consumed == anchor` component 후보와 suffix 전체가 조사 DFA를 통과해야 하는 `consumed == token` 후보를 분리한다. compound의 뒤 component를 조사 continuation으로 강제하지 않는다.
- `TokenAnalysisGraph`는 `source-whole`, `source-component`, `runtime-composed`, `unknown` 근거를 구분한다. query를 지지하는 source-whole 근거는 runtime composition보다 높은 provenance로 기록하지만 query와 무관한 source-whole 행이 satisfiable한 runtime 경로를 숨기지는 않는다. 같은 종류의 source 분석은 모두 보존하고 known complete path가 있으면 unknown path를 모순 근거로 사용하지 않으며 진단 순서는 structural identity로 결정한다.
- known complete path가 없을 때 candidate별 hybrid path는 query core 시작 전의 prefix 중 해당 시작 위치에 source hit가 없는 구간만 unknown node로 연결할 수 있다. source hit가 있는 위치의 금지된 categorical transition은 unknown으로 우회하지 않는다. core를 덮는 lexical identity, core 이후의 anchor·continuation과 context proof는 모두 source node에서 나와야 하며 unknown node는 lexical·POS·continuation·context 근거가 되지 않는다. 이 제한을 만족하는 complete path가 없으면 `UnknownOnly` 또는 `NoCompletePath`로 남긴다.
- bounded context는 이전·현재·다음 token graph에 추가하는 구조 제약이며 query pattern을 만족할 수 있는 분석만 줄인다. 반복 token과 인접 POS sequence는 lexical surface registry 없이 token 동일성·완전 POS sequence·시작 POS 집합으로 선언한다. context selector는 token 관계와 complete-path POS 존재 여부를 먼저 확인하고 가능한 selector만 graph traversal로 검증한다. query와 무관한 whole-token 동형 분석의 존재만으로 지지 분석을 제거하지 않는다.
- 현재 token에 체언 host와 조사 suffix의 유일한 split이 있으면 host 전체만 nominal particle context로 선택한다. 같은 token의 adverb whole 분석과 host 내부 strict component는 제거하며 split이 둘 이상이면 하나를 강제하지 않는다.
- runtime-composed path는 source expression과 multi-POS row에서 build 시점에 파생한 categorical `end_pos -> start_pos` edge로만 node를 연결한다. dense connection matrix, connection cost와 word cost는 structural artifact와 resolver 입력에 포함하지 않는다.
...
- resolver는 query와 무관한 complete path를 먼저 열거하지 않는다. source node와 component의 단일-node lexical proof는 complete-path node에서 직접 판정하고 multi-node lexical traversal은 생산적 파생을 허용하는 pattern에만 실행한다. start/end reachability를 보존한 packed token DAG와 query lexical/continuation automaton을 교차하고, node 상한과 지지 proof 상한은 query와 무관한 segmentation 조합 수가 아니라 실제 source node와 서로 다른 지지 proof에 적용한다.
...
- multi-node lexical support는 평탄화된 query lexical form을 source POS sequence에 직접 대입하지 않는다. resolver는 query lexical form도 같은 structural graph로 known-only 분석해 canonical surface·POS unit trace를 준비하고, query fine POS에 맞는 lexical automaton을 통과한 trace와 source core trace가 일치할 때만 lexical identity proof를 만든다. query와 source의 fused·unaligned node는 원문 byte span이 아니라 canonical component 순서로 정렬하며 source continuation은 일치한 lexical trace 뒤에서 별도로 검증한다.
- query lexical trace는 현재 token graph에 query core를 채우는 둘 이상의 source node 경로가 있을 때만 준비하고 같은 `PreparedQueryAnalysis`의 모든 candidate가 공유한다. single-node source proof만 가능한 candidate는 query graph를 만들지 않으며 lazy 준비 여부가 resolver 결과를 바꾸지 않아야 한다.
- query trace 준비는 lexical form의 시작점에서 source common-prefix와 categorical hard edge를 교차하는 query-directed traversal로 실행한다. 시작점에서 도달할 수 없거나 lexical automaton이 거부한 node와 complete path를 materialize하지 않으며 동일한 canonical trace 집합과 node·trace 상한을 보존한다.
- compact decision의 runtime lexical support는 source graph를 query trace prefix와 함께 순회하며 검증한다. 중간 candidate path 목록을 만들거나 path와 pattern의 곱을 다시 스캔하지 않고 동일한 canonical support 집합과 path 상한을 보존한다.
- pattern별 query trace 집합은 결정적 prefix trie로 준비한다. compact traversal은 source unit을 추가할 때 현재 trie 상태만 증분 전이하고 이미 수용한 prefix나 전체 trace 집합을 다시 비교하지 않으며, 한 source node 안에서 끝나는 짧은 trace와 긴 trace의 terminal을 모두 동일한 canonical support로 보존한다.
- graph resource decoder는 surface와 POS의 string-table identity를 analysis와 component에 보존한다. 같은 resource에서 준비한 source graph와 query trace는 compact traversal에서 이 identity를 정수로 비교하고, 문자열 값은 진단 proof와 외부 표현에만 사용한다.
- compact context selection은 current token의 copular split을 `PreparedTokenSummary`의 DAG fact로 memoize하고 adjacent token의 POS sequence를 automaton state traversal로 판정한다. 전체 witness path와 proof node는 진단 요청에서만 materialize하며 같은 complete-path context 결론을 보존한다.
- query lexical trace automaton은 nominal compound, 생산적 `XSV`·`XSA` 파생과 predicate·auxiliary chain을 구조 범주로 표현한다. surface별 예외, corpus 비용과 source 쪽 POS sequence 추측은 포함하지 않으며 query graph에 complete trace가 없으면 기존 single-node lexical proof만 사용할 수 있다.
- nominal compound lexical support는 query core가 token 시작에서 시작하고 query lexical form이 연속된 둘 이상의 source nominal node surface 전체와 정확히 같을 때만 host identity proof가 된다. query core 앞의 component나 lexical form 뒤의 비조사 nominal node를 포함하는 strict substring은 이 proof로 승격하지 않는다.
- predicate candidate의 `core`가 token prefix일 때 matcher의 `consumed`가 token 전체이더라도 nominalizer-particle branch가 아닌 같은 complete source path가 술어 경로를 `ETM`으로 종결하고 나머지를 하나 이상의 nominal node와 선택적 `XSN`·particle chain으로 token 끝까지 완결하면 attached nominal frame proof가 된다. 같은 시작점의 더 긴 frame-compatible source node는 짧은 분해를 지배한다. token 시작에서 `core`보다 길게 이어지는 predicate source lexical node 또는 atomic nominal source node가 있고 같은 lexical span의 어떤 source analysis도 query lexical identity를 포함하지 않거나 경쟁하는 whole-token lexical identity가 있으면 attached frame이 이를 덮지 않는다. frame의 nominal surface는 registry로 제한하지 않으며 predicate `core`와 nominal context span을 합치지 않는다.
- fused·unaligned component는 내부 byte span을 만들지 않지만 query anchor와 반환 span이 enclosing source node 전체를 포함하면 canonical component sequence를 lexical identity proof로 사용할 수 있다. nominal particle transition을 선언한 predicate branch에서는 lexical component 뒤의 span 없는 `ETN` component가 enclosing node 안에 있을 때 component ordinal 순서로 anchor morphology DFA를 nominalized 상태까지 전진시키며, query anchor의 다른 ending을 외부 continuation으로 다시 소비하지 않는다. 이 전이는 반환 span이나 외부 continuation coverage를 확장하지 않으며 strict component subspan을 반환해야 할 때만 opaque ambiguity로 남긴다.
- 제품 판정은 structural decision, query-scoped bounded context와 ambiguity disambiguation의 세 계층으로 실행한다. `Supported`는 수용하고 `Contradicted`는 거부하며 `Ambiguous`의 안정된 지지 분석만 별도 `AmbiguityResolver`가 선택한다. disambiguator는 graph가 지지하지 않은 분석을 만들 수 없고 surface별 allowlist, lexical context registry 또는 morphology 비용 임계값을 다른 형식으로 복원하지 않으며 confidence가 부족하면 abstain한다.
- 제품 hot path의 `ConstraintDecision`은 outcome, 안정된 support kind와 ambiguity class만 만들고 node 문자열, witness path와 진단용 proof를 복제하지 않는다. 명시적 진단 요청은 같은 prepared graph에서 `ConstraintResolution`을 만들며 두 경로의 outcome과 제품 수용 결과가 일치해야 한다.
- 정규화된 current token의 source node, categorical edge와 reachability는 `PreparedTokenAnalysis`로 한 번 만들고 같은 token의 모든 candidate가 공유한다. 명사+조사 host처럼 current token 구조만으로 결정되는 context fact는 이를 소비할 수 있는 candidate가 처음 요청할 때 prepared token summary에 가능한 range 집합으로 한 번 계산하며 후보 없음과 복수 후보를 같은 값으로 접지 않는다. candidate decision은 자신의 core·consumed 범위와 일치하는 fact만 소비하고 complete witness path를 다시 열거하지 않는다. 이전·다음 token graph는 pattern이 adjacent constraint를 선언할 때만 준비하며 cache는 resource identity와 token을 key로 사용하는 고정 entry·byte 상한을 가져 corpus 크기에 비례해 증가하지 않는다.
- `PreparedTokenAnalysis`의 known node는 schema 4 string table과 정규화 token을 빌려 쓰고 candidate마다 surface·POS 문자열을 복제하지 않는다. categorical transition은 resource가 부여한 정수 POS class와 dense `end_pos -> start_pos` bit matrix로 조회하고 compact decision은 query-constrained traversal에서 지지 proof 식별자를 직접 수집하며 중간 candidate path 집합을 다시 pattern별로 분석하지 않는다. continuation traversal은 시작 lexical path의 unit stream을 한 번 만들고 successor unit을 제자리에서 추가·되돌리며 부분 path마다 unit stream과 support를 다시 만들지 않는다. prefix 판정은 borrowed suffix view의 coverage와 POS를 직접 소비하고 최종 지지 proof가 채택될 때만 continuation unit을 materialize한다. compact decision에 필요 없는 reverse edge, `SupportedAnalysis`, continuation unit과 witness path는 진단 요청에서만 materialize한다.
- 공식 constraint evaluator의 성능 process는 compact decision과 정책 결과만 만들고 corpus 크기에 비례한 상세 proof를 생성하거나 보관하지 않는다. product control은 component resource를 로드하는 별도 process에서 case별 span만 넘기고, diagnostic parity도 별도 process에서 `ConstraintResolution`을 materialize해 decision과 비교하며 두 process의 시간과 peak RSS는 제품 후보 성능에서 제외한다. node·path proof가 필요한 명시적 진단은 graph shadow 진단 경로로 분리한다.
- reference candidate enumerator는 query branch anchor만 공유하고 기존 verifier, boundary 판정, lexical context registry와 비용 lattice를 호출하지 않는다. 평가는 candidate coverage, resolver conditional quality, ambiguity·unavailable, 정책별 품질과 기존 제품 disagreement를 분리한다.
- verifier가 소비한 candidate token 자체가 surrounding token의 strict subspan이고 query를 지지하는 source component 또는 runtime decomposition이 있을 때 `CompoundExposure` ambiguity다. predicate 활용이나 조사 연쇄처럼 candidate core는 strict subspan이어도 verifier가 candidate token 전체를 소비했다면 compound exposure로 분류하지 않는다. `opaque`는 거부하고 `transparent`는 수용하며 `explicit`은 query가 별도 component 노출 capability를 선언한 경우만 수용한다. 세 profile 중 품질 채택 조건을 통과한 profile이 없으면 제품 전환 실패로 기록한다.
...
- 계층형 제품 후보의 full-POS morphology benchmark는 prepared token graph와 candidate decision 시간을 분리해 기록하고, 같은 revision의 제품 control 대비 cases/s 10% 이상 저하, p95 10% 이상 증가 또는 RSS 20% 이상 증가가 없어야 한다. 이 성능 gate와 기존 품질 gate를 모두 통과한 뒤에만 legacy matcher fallback 없이 `ConstraintDecision`과 `AmbiguityResolver`를 제품 경로로 전환한다.
...
-아요/-어요와 -았어요/-었어요
-면/으면, -며/으며, -니/으니, -니까/으니까, -니까는/으니까는, -니깐/으니깐, -더라도, -려고/으려고, -리라고/으리라고
-아/어가고, -아/어가야
-ㅂ니다/습니다
-기, -음/ㅁ
```
...
표면형과 fine POS가 typed context rule registry에 등록되어 반복 token 문맥 규칙으로 판별할 수
있을 때만 `LexicalContext`와 compact component resource를 요구한다.
...
typed context rule registry에 등록된 `매일/MAG`의 `smart` 부사 matcher로 검색한다. 각 candidate의
