...
- gold corpus에 포함된 현재 평서형 `-ㄴ다/는다`와 상태 용언의 `-다`, 회상 관형형 `-던`, 양보 연결형 `-더라도`, 과거 관형 연쇄 `-았/었을`, 과거 의문 종결 연쇄 `-았/었느냐`, `-았/었느냐는`, 이유 연결형 `-(으)니`, 동시 연결형 `-(으)면서`, 인용 연결형 `-다고`, 현재 서술형 인용·회상·조건 연쇄, 전망 인용 연쇄 `-(으)리라고`, 의도 연결형 `-(으)려고`, 상태 변화 보조 용언 `-아/어지다`, 진행 방향 보조 용언 `-아/어가고`, `-아/어가야`도 v0.1의 제한된 continuation vocabulary에 포함한다.
...
- 동시 연결형 `ending.coordinate-myeonseo`는 `-면서/-으면서`를 완성된 predicate
  token으로 소비한다. 받침 없는 어간과 `ㄹ` 받침 어간은 `-면서`, 그 밖의 규칙 받침 어간은
  `-으면서`를 쓴다. 불규칙 용언은 기존 어간 교체를 먼저 적용해 `걷다 -> 걸으면서`,
  `돕다 -> 도우면서`를 만들고 `살다 -> 살면서`를 허용한다. `걸다 -> 걸으면서`처럼 다른
  표제어로 귀속하지 않으며, 같은 `걷다`의 규칙형 동형어가 만드는 `걷으면서`는 별도 분석으로
  보존한다.
...
-아요/-어요와 -았어요/-었어요
-면/으면, -며/으며, -면서/으면서, -니/으니, -니까/으니까, -니까는/으니까는, -니깐/으니깐, -더라도, -려고/으려고, -리라고/으리라고
-아/어가고, -아/어가야
-ㅂ니다/습니다
-기, -음/ㅁ
```
