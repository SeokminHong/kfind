...
- 라이브러리 matcher는 UTF-8 byte slice에서 겹치지 않는 match와 형태 분석 provenance를
  ...
  포함하지 않는다.
- `kfind`, `kfind-data`, `kfind-morph`, `kfind-query`, `kfind-matcher`는 Rust 1.85에서
  `wasm32-unknown-unknown` 대상으로 빌드되어야 한다.
- WASM 지원은 Rust target 호환성을 의미한다. JavaScript binding과 패키징은 별도 API
  설계 전까지 범위에 포함하지 않는다.
- 기본 CI는 Linux와 Apple Silicon macOS에서 네이티브 테스트를 실행하고, Linux에서
  MSRV의 WASM library build를 검사한다.
...
-는, -(으)ㄴ, -(으)ㄹ
-아/-어, -아서/-어서
-았/-었, -겠, -시. 존대 선어말어미는 받침 어간뿐 아니라 모음·ㄹ·불규칙 어간의 올바른 교체형에도 결합한다.
-아요/-어요와 -았어요/-었어요
-면/으면, -며/으며
-ㅂ니다/습니다
-기, -음/ㅁ
