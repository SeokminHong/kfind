...
- Resource의 raw POS sequence는 token graph 준비 단계에서 compact typed sequence arena로 한 번
  변환한다. Edge는 arena range만 보존하고 구조 상태기계는 raw 문자열을 기능마다 다시 분리하거나
  세부 품사를 반복 해석하지 않는다. 같은 resource POS 문자열은 token graph 안에서 같은 sequence를
  공유하며 intern 상태와 arena는 edge node 상한으로 제한한다.
...
- compact morphology resource는 schema 5 container다. NFC surface index, source node의
  ...
  같은 문자열을 다시 소유하지 않는다. 구조 판정용 typed POS sequence는 token 준비 중 임시 arena에
  compact code로 변환하고 준비 결과에는 raw 문자열이나 arena를 보존하지 않는다.
...
계약 보정 지표만으로 정밀도 회귀가 없다고 주장하지 않는다. kfind 품질 표와 차트는 같은 profile의
raw TP·FP·TN·FN·precision·recall과 TPᶜ·FPᶜ·TNᶜ·FNᶜ·precisionᶜ·recallᶜ를 나란히 표시한다.
FPᶜ·FNᶜ·recallᶜ는 annotation과 제외가 실제 적용된 제품 계약 값이어야 하며 strict 값의 별칭으로
표시하지 않는다. 외부 분석기 비교의 기본 순위는 strict corpus gold를 사용한다.
...
POS 문자열을 고유하게 만들어 token 내부 POS interning hit가 없는 대조군을 측정한다. 반복 POS만
쓰는 dense workload와 함께 비교해 typed POS sequence 변경을 비정상적으로 높은 intern hit율로
정당화하지 않는다.
