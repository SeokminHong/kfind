...
- resolver 뒤 제품 정책은 `whole`, `explicit-component`, `possible-analysis`로 분리한다. 형태 구조만으로 여러 lexical identity나 의미가 함께 지지되면 가능한 분석을 보존하며 통계적 또는 문맥적 disambiguator 없이 의도한 의미 하나를 자동 선택하지 않는다.
