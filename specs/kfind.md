...
- 정규화된 current token의 source node, categorical edge와 reachability는 `PreparedTokenAnalysis`로 한 번 만들고 같은 token의 모든 candidate가 공유한다. 이전·다음 token graph는 pattern이 adjacent constraint를 선언할 때만 준비하며 cache는 resource identity와 token을 key로 사용하는 고정 entry·byte 상한을 가져 corpus 크기에 비례해 증가하지 않는다.
...
- 계층형 제품 후보의 full-POS morphology benchmark는 같은 revision의 제품 control 대비 cases/s 10% 이상 저하, p95 10% 이상 증가 또는 RSS 20% 이상 증가가 없어야 한다. 이 성능 gate와 기존 품질 gate를 모두 통과한 뒤에만 legacy matcher fallback 없이 `ConstraintDecision`과 `AmbiguityResolver`를 제품 경로로 전환한다.
