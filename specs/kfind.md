...
- 공식 constraint evaluator의 성능 process는 compact decision과 정책 결과만 만들고 corpus 크기에 비례한 상세 proof를 생성하거나 보관하지 않는다. 같은 fixture의 별도 diagnostic parity process가 `ConstraintResolution`을 materialize해 decision과 비교하며 이 process의 시간과 peak RSS는 제품 후보 성능에서 제외한다. node·path proof가 필요한 명시적 진단은 graph shadow 진단 경로로 분리한다.
