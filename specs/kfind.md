...
- `PreparedTokenAnalysis`의 known node는 schema 4 string table과 정규화 token을 빌려 쓰고 candidate마다 surface·POS 문자열을 복제하지 않는다. categorical transition은 `end_pos -> start_pos` 색인으로 조회하고 compact decision은 query-constrained traversal에서 지지 proof 식별자를 직접 수집하며 중간 candidate path 집합을 다시 pattern별로 분석하지 않는다. compact decision에 필요 없는 reverse edge, `SupportedAnalysis`, continuation unit과 witness path는 진단 요청에서만 materialize한다.
