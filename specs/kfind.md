...
- 공식 constraint evaluator는 compact decision과 정책 결과만 corpus 크기에 비례해 보관한다. decision·diagnostic parity 검증을 위해 만든 상세 proof는 후보 검증 직후 해제하고 benchmark JSON에 복제하지 않으며, node·path proof가 필요한 명시적 진단은 graph shadow 진단 경로로 분리한다. evaluator peak RSS와 처리량은 이 compact 제품 후보 수명을 측정한다.
