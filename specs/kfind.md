...
- known complete path가 없을 때 candidate별 hybrid path는 query core 시작 전의 prefix만 unknown node로 연결할 수 있다. core를 덮는 lexical identity, core 이후의 anchor·continuation과 context proof는 모두 source node에서 나와야 하며 unknown node는 lexical·POS·continuation·context 근거가 되지 않는다. 이 제한을 만족하는 complete path가 없으면 `UnknownOnly` 또는 `NoCompletePath`로 남긴다.
