...
- fused·unaligned component는 내부 byte span을 만들지 않지만 query anchor와 반환 span이 enclosing source node 전체를 포함하면 canonical component sequence를 lexical identity proof로 사용할 수 있다. lexical component 뒤의 span 없는 canonical component는 enclosing node가 query anchor 안에 있을 때만 component ordinal 순서로 anchor morphology DFA를 전진시키고, 반환 span이나 외부 continuation coverage를 확장하지 않는다. strict component subspan을 반환해야 할 때만 opaque ambiguity로 남긴다.
