...
- resolver 결과는 `Proven`, `Contradicted`, `Ambiguous`, `Unavailable`과 proof다. 비용은 같은 종류의 근거 안에서 경로 순서와 진단에만 사용하며 비용 차이만으로 verdict를 바꾸지 않는다. resolver는 query 제약을 만족하는 분석 경로의 존재를 판정하므로 query와 무관한 다른 품사·분해 경로는 그 자체로 모순이 아니며, span 관계를 구조적으로 하나로 정할 수 없을 때만 `Ambiguous`를 반환한다.
...
- bounded context는 token graph에 추가하는 구조 제약이며 query pattern을 만족할 수 있는 경로만 줄인다. query와 무관한 whole-token 동형 분석의 존재만으로 `Ambiguous`를 만들지 않으며 lexical identity, fine POS, continuation과 인접 token 제약을 모두 만족하는 경로가 있으면 satisfiable하다.
