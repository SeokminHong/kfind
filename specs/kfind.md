...
- compact component projection은 benchmark에서 검증한 schema 1 container를 제품에도 그대로
  사용한다. full resource와 같은
...
- compact container는 schema, source SHA-256, section length·digest, UTF-8 POS table, payload offset,
  context ID, component scoring field와 matrix 범위를 제품 초기화에서 검증한다.
...
- 이 비교를 위해 local lattice evaluator는 benchmark가 구현할 수 있는 읽기 전용 resource
  계약만 받는다. 제품 matcher와 benchmark는 이 evaluator를 공유한다.
...
- shadow 동등성 게이트 이후 제품 loader는 compact schema 1을 그대로 사용한다. 제품 초기화는
...
gold reject 15개 중 source 분해가 원문 표면과 맞지 않는 2개를 제외한 정상 VCP 13개는
Korean-Kaist·KSL dev에서 같은 실패 원인을 먼저 분류한다. 비용·threshold 후보는 dev에서만
정하고, 기본 검색 결과를 바꾸기 전에 별도 unseen source를 고정한다. 그 전에는 P3 filtering을
적용하지 않는다.
