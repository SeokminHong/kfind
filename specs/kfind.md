...
- bounded context는 이전·현재·다음 token graph에 추가하는 구조 제약이며 query pattern을 만족할 수 있는 분석만 줄인다. 반복 token과 인접 POS sequence는 lexical surface registry 없이 token 동일성·완전 POS sequence·시작 POS 집합으로 선언한다. query와 무관한 whole-token 동형 분석의 존재만으로 지지 분석을 제거하지 않는다.
