...
- `smart` 체언 query가 token 왼쪽 경계부터 시작하고 query program이 하나 이상의 조사를
  ...
  graph에 같은 `J+VCP+E+` 분할을 중복 요구하지 않는다. 따라서 `대학뿐이다`,
  `대학뿐만이다`처럼 보조사를 포함한 명사구 뒤 지정사를 일반적으로 지원하되, 허용되지 않은
  조사 연쇄나 지정사·어미가 완결되지 않은 표면은 열지 않는다.
...
- 조사 연쇄는 optional `들`을 포함해 최대 네 규칙을 순회하며,
  `data/rules/particles.toml`의 `next` 전이만 허용한다. 첫 조사는 격 조사와 보조사 중 어느
  쪽도 될 수 있고, 이후 조사의 종류도 고정된 slot 순서가 아니라 직전 규칙의 `next`로
  결정한다. 따라서 `까지 → 도/만/은·는` 전이는 `까지도`·`까지만`·`까지는` 계열을 함께
  설명하고, `부터 → 의` 전이는 `으로부터의`를 설명한다. 목록에 없는 역순과 최대 길이를
  넘는 연쇄는 거부한다.
...
- pinned 국립국어원 조사 catalog에서 한국어기초사전과 표준국어대사전이 함께 지지하는
  ...
  `뿐`, `처럼`, `커녕`은 독립 원자 규칙으로 유지한다. `이나/나`, `이나마/나마`,
  ...
  이 원자들 뒤의 추가 조사는 각 규칙의 `next`로만 허용한다.
...
5. `gold-or-adapter`: gold lemma·품사·정렬이나 외부 adapter의 정규화가 제품 계약과 맞는지
   먼저 수정·확정해야 한다.
6. `out-of-contract`: derivation, 비표준 띄어쓰기, 미지원 외래 고유명사처럼 현재 profile의
   명시적 범위 밖이다.

사전 증거는 고정한 snapshot의 구조화된 표제어·품사·활용·어휘 관계 필드만 사용한다.
