...
- resolver는 query 제약을 만족하는 모든 분석을 `SupportedAnalysisSet`과 proof로 반환하고 `Contradicted`, `Ambiguous`, `Unavailable`을 별도 결과로 보존한다. 비용은 같은 종류의 근거 안에서 경로 순서와 진단에만 사용하며 dense connection matrix의 셀 존재 여부나 비용 차이로 hard edge 또는 제품 수용 여부를 바꾸지 않는다. query와 무관한 다른 품사·분해 경로는 그 자체로 모순이 아니며 span 또는 component 노출 관계를 구조적으로 정할 수 없을 때만 `Ambiguous`를 반환한다.
...
- graph schema 3은 schema 2의 source analysis와 relation projection에 source expression과 multi-POS row에서 파생한 categorical morphotactic transition table을 추가한다. 전체 constraint resolver는 schema 3만 사용하고 schema 2 또는 dense connection matrix의 셀 존재 여부로 runtime path를 fallback하지 않는다.
...
- `TokenAnalysisGraph`는 `source-whole`, `source-component`, `runtime-composed`, `unknown` 근거를 구분한다. query를 지지하는 source-whole 근거는 runtime composition보다 높은 provenance로 기록하지만 query와 무관한 source-whole 행이 satisfiable한 runtime 경로를 숨기지는 않는다. 같은 종류의 source 분석은 비용으로 하나를 고르지 않고 known complete path가 있으면 unknown path를 모순 근거로 사용하지 않으며 연결 비용과 word cost는 같은 근거 종류 안의 진단 순서에만 사용한다.
