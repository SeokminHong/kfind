...
`-이-/-히-/-리-/-기-` 피동 파생을 직접 가리키는 경우도 기본 `inflection`에 포함한다. 이는
모든 동사에 피동 접미사를 생산적으로 붙이는 규칙이 아니라 사전이 확인한 lemma 관계다.
...
사전 피동 파생 관계도 같은 enriched TSV의 `SurfaceOnly` 분석에
`lexical.dictionary-passive=target-lemma`로 저장한다. 이 행은 base의 full POS 분석을 가리지 않고
...
피동 접미사를 추측하거나 target 표면을 base의 단순 alias로 취급하지 않는다.
