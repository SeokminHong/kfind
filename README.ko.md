...
규칙·사전에 포함된 불규칙 활용을 검색합니다. `derivation`은 `-적`, `-하다`,
`-되다`, `-시키다` 등 등록된 생산적 파생을 추가합니다. `literal`은 형태 확장을
비활성화합니다.
...
| 정책 | 동작 | 주 용도 |
| --- | --- | --- |
| `smart` | 품사별 검증을 적용하고 완성된 token span의 경계를 확인합니다. 합성 명사와 어휘 용언에 선택적 component resource를 사용할 수 있습니다. | 대화형 검색, 기본값 |
...
| 옵션 | 기본값 | 설명 |
| --- | --- | --- |
| `--data-dir <PATH>` | 자동 탐색 | 하나의 명시적 디렉터리에서 `lexicon.bin`과 `morphology-component-compact.kfc`를 읽습니다. |
...
핵심 불규칙 용언과 규칙은 binary에 포함됩니다. Homebrew는 고정된 full POS lexicon과
compact 형태 component resource도 `share/kfind`에 설치하므로 실행 중 네트워크 접근이
필요하지 않습니다.
...
직접 선택할 수 있습니다. `--embedded`는 full POS 탐색만 건너뜁니다. Compile된 `smart`
plan에 component 근거가 필요하면 component resource는 계속 찾아 검증하며, 필요하지 않은
plan은 이를 로드하지 않습니다.

고정되고 checksum 검증을 거친 `mecab-ko-dic` 소스에서 full POS 산출물을 재현할 수 있습니다.
...
| --- | ---: | ---: | ---: | ---: |
| Agent: embedded + `any` + explicit POS | 479 / 11 / 21 | 16.1 ms | 6,193.6 MiB/s | 7.0 MiB |
| Human: full POS + `smart` + untagged | 410 / 0 / 90 | 301.5 ms | 331.6 MiB/s | 91.5 MiB |
...
workflow를 설명합니다. Human 행은 2026-07-14 후보 revision `10dd69a`의 결과이며, 변경되지
...
Agent와 User는 2026-07-14에 측정했습니다. 외부 행은 fixture, schema, 버전과 설정이 바뀌지
않은 고정 snapshot을 재사용합니다.

| backend | 입력·버전 | TP / FP / FN | precision | recall | F1 | init | cases/s | p95 | peak RSS |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Agent | embedded + `any`, 품사 명시 | 479 / 11 / 21 | 97.76% | 95.80% | 96.77% | 0.0011초 | 15,821.7 | 0.1431 ms | 5.3 MiB |
| User | full POS + `smart`, 품사 생략 | 410 / 0 / 90 | 100.00% | 82.00% | 90.11% | 0.4254초 | 11,893.6 | 0.2068 ms | 92.0 MiB |
...
라이브러리와 핵심 의존 crate는 Rust 1.85의 `wasm32-unknown-unknown` target을 지원합니다.
...
형태론 fixture에는 일치·불일치 회귀 사례 452개가 있습니다. Docker 벤치마크는 독립된 UD
Korean-Kaist·KSL test split에서 생성한 1,000개 사례로 `kfind`를 측정하고, 고정된 Kiwi,
Lindera, MeCab-ko, KOMORAN snapshot과 비교합니다. 쿼리 parsing과 잘못된 matcher 입력을 위한
fuzz target은 `fuzz/`에 있습니다.
