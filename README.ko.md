...
`kfind`는 쿼리를 한 번 분석해 개수가 제한된 표면형 앵커와 verifier로 컴파일하고,
...
Pager index는 완성된 source line과 match별 전개 row 수에 비례합니다. 2026-07-14 revision
`4d81a22`에서 plain 100만 줄은 peak RSS 43.50 MiB, source 100만 줄이 400만 row로 전개된
입력은 110.81 MiB를 사용했습니다. 수백만 결과의 interactive 탐색이 필요하지 않으면
`--no-pager`로 bounded stream을 사용합니다. 자세한 값은
[TUI index memory 보고서](docs/benchmarks/2026-07-14-tui-index-memory.md)에 있습니다.
...
| 코드 | 의미 |
| ---: | --- |
| `0` | 하나 이상의 match를 찾았거나 agent 초기화에 성공했습니다. |
| `1` | match가 없습니다. |
...
없는 130개만 저장합니다. 한국어기초사전에서 용언과 부사의 entry ID가 양방향으로 일치하는
파생형 153개도 저장합니다. 결과 TSV는 283개 surface-only 행을 포함하며 27,707바이트입니다.
활용형은 기본 `inflection`, 파생 부사는 `derivation`에서만 사용합니다.

## 벤치마크

아래 결과는 서로 다른 workload를 요약한 것입니다. 형태 품질, end-to-end CLI 처리량,
resource 초기화와 literal scan을 하나의 점수로 합치지 않습니다. 링크된 보고서에는 환경,
입력 digest, revision, warm-up과 반복 횟수가 기록되어 있습니다.

### 제품 workflow

최신 형태소 측정은 Linux/aarch64에서 한 번의 warm-up 뒤 fresh process 5회의
...
workflow를 설명합니다. 제품 행은 2026-07-15 후보 revision `ccc9525`의 결과입니다.

교차 검증된 사전 활용형 12,888개 중 12,758개는 생산 규칙으로 생성하고, 나머지 130개만
enriched 데이터에 저장합니다. 양방향 용언·부사 파생 관계 153개도 저장해 전체 표면형 계층은
27,707바이트입니다. 새 hard-negative FP 없이 Agent와 Human FN을 각각 1건 줄였습니다. Agent
morphology cases/s는 1.05% 낮았고 Human morphology와 두 CLI wall 측정 범위는 겹쳤습니다.

`ending.connective-eudoe` verifier는 과거·미래 선어말어미 state 뒤에서만 `으되`를 소비합니다.
`치르다 -> 치렀으되`를 복구해 development embedded와 full-POS `smart`의 FN을 각각 1건
줄였습니다. 고정 test, Agent, Human과 hard-negative 품질은 바뀌지 않았고 성능 범위는
기준선과 겹쳤습니다.

현재 서술형 verifier는 `-ㄴ다/-는다` 뒤의 `고`, `는`, `던`, `면`, `니`, `며`, `면서`,
`는데`, `지`를 소비합니다. `쓰다 -> 쓴다고`를 복구해 development embedded와 full-POS
`smart`의 FN을 각각 1건 줄였고 신규 FP는 없었습니다. 고정 test, Agent, Human과
hard-negative 품질은 바뀌지 않았습니다. Morphology cases/s는 2.88~4.04% 낮았고
초기화와 RSS는 유지됐습니다.

- [2026-07-15 현재 서술형 후속 형태 continuation](docs/benchmarks/2026-07-15-present-declarative-continuation.md)
- [2026-07-15 선어말어미 뒤 `-으되` continuation](docs/benchmarks/2026-07-15-eudoe-continuation.md)
- [2026-07-15 제한된 사전 표면형 계층](docs/benchmarks/2026-07-15-dictionary-surface-lexicon.md)
...
2026-07-14 revision `46f9ceb`에서 한국어 README·소스 코드 주석·기술 문서 25건을 고정해
다섯 slice를 평가했습니다. fixture는 positive 21건, negative 4건인 진단용 소표본이므로
제품 전체 품질 점수나 profile 순위로 해석하지 않습니다.
...
User의 띄어쓰기 오류 slice recall은 20%였습니다. query와 gold span은 첫 실행 전에 고정했고,
이 결과는 UD 회귀 fixture를 대체하거나 규칙 선택에 사용하지 않습니다.

- [2026-07-14 현실 기술 코퍼스 blind 평가](docs/benchmarks/2026-07-14-real-corpus-blind.md)

### 외부 분석기 비교

아래 표는 같은 1,000-case explicit-POS fixture와 gold를 사용합니다. Agent와 고정 외부
분석기는 품사를 명시하고, User는 대화형 사용을 반영해 같은 query에서 품사를 제거합니다.
Agent와 User는 2026-07-15에 측정했습니다. 외부 행은 fixture, schema, 버전과 설정이 바뀌지
않은 고정 snapshot을 재사용합니다.

| backend | 입력·버전 | TP / FP / FN | precision | recall | F1 | init | cases/s | p95 | peak RSS |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Agent | embedded + `any`, 품사 명시 | 482 / 11 / 18 | 97.77% | 96.40% | 97.08% | 0.0012초 | 14,450.3 | 0.1551 ms | 5.2 MiB |
| User | full POS + `smart`, 품사 생략 | 420 / 0 / 80 | 100.00% | 84.00% | 91.30% | 0.4323초 | 10,404.8 | 0.2348 ms | 92.2 MiB |
...
| workload | 결과 | 측정 |
| --- | --- | --- |
| 1 GiB warm-cache no-hit literal scan | 중앙값 0.047초, 21,787 MiB/s, RSS 7.23 MiB | 2026-07-12, revision `a7b3c28`; 함께 측정한 `rg -F`의 중앙 wall·처리량과 같음 |
| 632,667개 entry의 full POS native warm startup | 0.08초, peak RSS 39.5 MiB | 2026-07-13, revision `8845f33` |
| Query compile p95 | atom 1개 0.077875 ms, atom 8개 0.153838 ms | 2026-07-11, revision `026ff81` |

1 GiB 결과는 low-hit I/O 회귀 검사이며 형태 검색과 `rg -F`의 기능이 같다는 의미가 아닙니다.

- [1 GiB mixed corpus 보고서](docs/benchmarks/2026-07-12-1gib-mixed.md)
- [Full POS startup 보고서](docs/benchmarks/2026-07-13-full-pos-startup.md)
- [Query compile 보고서](docs/benchmarks/2026-07-11-query-compile.md)
- [벤치마크 계약과 보고서 인덱스](docs/benchmarks/README.md)
...
형태론 fixture에는 일치·불일치 회귀 사례 540개가 있습니다. Docker 벤치마크는 독립된 UD
...
일치하는 `vX.Y.Z` tag를 push하면 릴리스 workflow가 실행됩니다. Full POS resource를 다시
빌드하고 검증한 뒤 source·data·CLI 산출물을 게시하고 `SeokminHong/homebrew-brew`에 Formula
PR을 엽니다. Tap의 `pr-pull` label은 Formula test가 통과한 뒤에만 적용됩니다.
